...
static void dump_UTS_power_on_reasons(void)
{
	char buffer[256] = {0};
	int index = boot_reason - 1;
	if (index >= ARRAY_SIZE(qpnp_pon_reason) || index < 0) {
		snprintf(buffer, 255, "###### Bootup Reason: %s ######\n",cold_boot ? "cold_boot" : "warm_boot");
	} else {
		snprintf(buffer, 255, "###### Bootup Reason: %s ######\n",cold_boot ? qpnp_pon_reason[index] : "warm_boot");
	}
	printk(KERN_NOTICE "%s", buffer);
	sys_write(g_hfileEvtlog, buffer, strlen(buffer));
}
