...
extern void begin_lock_microp_operation_wq(void);
...
int AX_MicroP_enterResuming(void){
    ...
    printk("%s ignore following i2c(0x15) error+++\r\n", __FUNCTION__);
...
// ASUS_BSP +++ Peter_Lu "For Pad I2C suspend/resume issue"
        if (g_i2c_bus_3v3_state == 0)		//only protect first trun on
            begin_lock_microp_operation_wq();
