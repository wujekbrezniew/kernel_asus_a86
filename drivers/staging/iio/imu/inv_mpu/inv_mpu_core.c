...
#define A86_GYRO_GPIO_IRQ_MPU6500_EVB   66

static int mpu6500_irq_gpio = 0;
static int mpu6500_fsync_gpio = 0;
struct inv_mpu_iio_s *g_MPU = NULL;

static __s8 orientation_sr1[9] = { 0, 1, 0, 1, 0, 0, 0, 0, -1 };
static __s8 orientation_sr2[9] = { 0, 1, 0, -1, 0, 0, 0, 0, 1 };
...
//ASUS_BSP +++ Jiunhau_Wang "[A86][Sensor][NA][Spec] use wq to process suspend/resume"
static struct workqueue_struct *g_mpu_wq = NULL;
static struct delayed_work g_mpu_resume_work;
//ASUS_BSP --- Jiunhau_Wang "[A86][Sensor][NA][Spec] use wq to process suspend/resume"
...
//ASUS_BSP +++ Jiunhau_Wang "[A86][Sensor][NA][ATD] support g-sensor and gyro status"
...
static int Test_MPU6500_SensorI2C(struct i2c_client *apClient)
{
	int lnResult = I2C_TEST_PASS;
	int result = 0;
	char data;
	
	i2c_log_in_test_case("Test_MPU6500_SensorI2C++\n");
	
	if(g_MPU == NULL)
		return I2C_TEST_FAIL_SENSOR;

	result = inv_i2c_read(g_MPU, 0x75, 1, &data);
	if (result < 0)
	{
		printk(KERN_INFO "[MPU6500][I2C] i2c read fail \n");
		return I2C_TEST_FAIL_SENSOR;
	}
	
	if(data != 0x70)
	{
		printk(KERN_INFO "[MPU6500][I2C] (0x%2x) != 0x70 \n", data);
		return I2C_TEST_FAIL_SENSOR;
	}
	printk(KERN_INFO "[MPU6500][I2C] ID = (%d) \n", data);
	i2c_log_in_test_case("Test_MPU6500_SensorI2C--\n");
	
	return lnResult;
}

static struct i2c_test_case_info gMPU6500_TestCaseInfo[] =
{
	__I2C_STRESS_TEST_CASE_ATTR(Test_MPU6500_SensorI2C),
};
...
//++++ ASUS Alian_Shen "change the max value of Accelermeter for Android 5.0: max = 4G"		
...
//ASUS_BSP +++ Jiunhau_Wang "[A86][Sensor][NA][Spec] Porting 9-axis sensor"
static ssize_t inv_gyro_id_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	char value = 0;
	struct inv_mpu_iio_s *st = iio_priv(dev_get_drvdata(dev));

	inv_i2c_read(st, 0x75, 1, &value);

	return sprintf(buf, "0x%x\n", value);
}
	
static DEVICE_ATTR(gyro_id, S_IRUGO, inv_gyro_id_show, NULL);

static ssize_t inv_gyro_i2c_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	char value = 0;
	int ret = 0;
...
static DEVICE_ATTR(gyro_i2c, S_IRUGO, inv_gyro_i2c_show, NULL);
//ASUS_BSP --- Jiunhau_Wang "[A86][Sensor][NA][Spec] Porting 9-axis sensor"
...
static DEVICE_ATTR(gyro_6500_status, S_IRUGO, read_gyro_status, NULL);
static DEVICE_ATTR(accel_6500_status, S_IRUGO, read_accel_status, NULL);
//ASUS_BSP --- Jiunhau_Wang "[A86][Sensor][NA][ATD] support g-sensor and gyro status"

//ASUS_BSP +++ Jiunhau_Wang "[A86][Sensor][NA][ATD] support Gdir and Mdir"
...
//ASUS_BSP +++ Jiunhau_Wang "[A86][Sensor][NA][ATD] support get accel raw data"
static ssize_t read_accel_raw(struct device *dev, struct device_attribute *devattr, char *buf)
{
	struct inv_mpu_iio_s *st = iio_priv(dev_get_drvdata(dev));
	u8 data[BYTES_FOR_DMP + QUATERNION_BYTES]; //32
	int result;
	int i = 0;
	short a[3];
	struct inv_reg_map_s *reg;
	...
	for (i = 0; i < ARRAY_SIZE(a); i++) {
		a[i] = be16_to_cpup((__be16 *)(&data[i * 2]));
...
DEVICE_ATTR(accel_6500_raw, S_IRUGO, read_accel_raw, NULL);
//ASUS_BSP --- Jiunhau_Wang "[A86][Sensor][NA][ATD] support get accel raw data"
...
//ASUS_BSP +++ Jiunhau_Wang "[A86][Sensor][NA][Spec] Porting 9-axis sensor"
	&dev_attr_gyro_id.attr,
	&dev_attr_gyro_i2c.attr,
//ASUS_BSP --- Jiunhau_Wang "[A86][Sensor][NA][Spec] Porting 9-axis sensor"
//ASUS_BSP +++ Jiunhau_Wang "[A86][Sensor][NA][ATD] support g-sensor and gyro status"
	&dev_attr_gyro_6500_status.attr,
	&dev_attr_accel_6500_status.attr,
//ASUS_BSP --- Jiunhau_Wang "[A86][Sensor][NA][ATD] support g-sensor and gyro status"
//ASUS_BSP +++ Jiunhau_Wang "[A86][Sensor][NA][ATD] support Gdir and Mdir"
	&dev_attr_Gdir.attr,
	&dev_attr_Mdir.attr,
//ASUS_BSP --- Jiunhau_Wang "[A86][Sensor][NA][ATD] support Gdir and Mdir"
//ASUS_BSP +++ Jiunhau_Wang "[A86][Sensor][NA][ATD] support get accel raw data"
	&dev_attr_accel_6500_raw.attr,
//ASUS_BSP --- Jiunhau_Wang "[A86][Sensor][NA][ATD] support get accel raw data"
#if CAL_DATA_AUTO_LOAD == 1
    &dev_attr_cal_data_auto_load.attr,
#endif
};
...
//ASUS_BSP +++ Jiunhau_Wang "[A86][Sensor][NA][Spec] patch for gyro data output consistency"
...
static irqreturn_t inv_mpu_interrupt_handler(int irq, void *dev_id)
{
	return IRQ_HANDLED;
}

// ASUS_BSP +++ Jiunhau "[A86][Sensor][NA][Spec] Porting 9-axis sensor(Part II)"
static int inv_mpu_gyro_fsysc_init(struct i2c_client *client)
{
	int ret = 0 ;
	
	mpu6500_fsync_gpio = of_get_named_gpio(client->dev.of_node, "mpu6500,fsync-gpio", 0);
	if(gpio_is_valid(mpu6500_fsync_gpio)){
		ret = gpio_request(mpu6500_fsync_gpio, "GYRO_FSYNC");
		if(ret)
			printk(KERN_INFO "[MPU6500] unable to request (mpu6500-fsync)\n");
	}
	
	ret = gpio_direction_output(mpu6500_fsync_gpio, 0);
	if (ret < 0) {
		printk(KERN_INFO "[MPU6500] not able to set gyro_fsync as output\n");
		return -1;
	}
	
	printk(KERN_INFO "[MPU6500] gyro fsync init complete!\n");
	return 0;
}

static int inv_mpu_gyro_irq_init(struct i2c_client *client, struct inv_mpu_iio_s *st)
{
	int result = 0;

	mpu6500_irq_gpio = of_get_named_gpio_flags(client->dev.of_node, "mpu6500,irq-gpio", 0, NULL);

	result = gpio_request(mpu6500_irq_gpio, "mpu6500-irq");
	if (result < 0){
		printk(KERN_INFO "[MPU6500] gpio %d request fail!\n", mpu6500_irq_gpio);
		goto init_irq_fail;
	}
	
	result = gpio_direction_input(mpu6500_irq_gpio);
	if (result < 0){
		printk(KERN_INFO "[MPU6500] gpio_direction_input fail!\n");
		goto init_irq_fail;
	}
	
	st->irq = gpio_to_irq(mpu6500_irq_gpio);
	if(st->irq < 0)
	{
		printk(KERN_INFO "[MPU6500] gpio_to_irq fail : %d\n", mpu6500_irq_gpio);
		return -1;
	}
	else
	{
		result = request_irq(st->irq,
					inv_mpu_interrupt_handler,
					IRQF_TRIGGER_RISING | IRQF_SHARED,
					"mpu6500_INT",
					&client->dev );
		if (result < 0){
			printk(KERN_INFO"[MPU6500][probe] (g_mpu6500_device.irq) request_irq() error %d.\n",result);
			return -1;
		}
	}	
	printk(KERN_INFO "[MPU6500] GPIO 66 init complete!\n");
	return 0;
	
init_irq_fail:
	gpio_free(mpu6500_irq_gpio);
	return result;
}
// ASUS_BSP --- Jiunhau "[A86][Sensor][NA][Spec] Porting 9-axis sensor(Part II)"

// ASUS_BSP +++ Jiunhau "[A86][Sensor][NA][Spec] support SR2"
static int inv_change_orientation(void)
{
	if(g_ASUS_hwID <= A86_SR1)
		memcpy(mpu6500_pdata.orientation, orientation_sr1, sizeof(mpu6500_pdata.orientation));
	else
		memcpy(mpu6500_pdata.orientation, orientation_sr2, sizeof(mpu6500_pdata.orientation));
	return 0;
}
// ASUS_BSP --- Jiunhau "[A86][Sensor][NA][Spec] support SR2"
//ASUS_BSP +++ Jiunhau_Wang "[A86][Sensor][NA][Spec] use wq to process suspend/resume"
static void mpu_resume_work(struct work_struct *work)
{
	int ret = 0;
	printk(KERN_INFO "[MPU6500] resume_work +++\n");
	ret = g_MPU->set_power_state(g_MPU, true);
	if(ret)
	{
		printk(KERN_INFO "[MPU6500] resume retry\n");
		queue_delayed_work(g_mpu_wq, &g_mpu_resume_work, msecs_to_jiffies(2));
	}
	printk(KERN_INFO "[MPU6500] resume_work ---\n");
}
//ASUS_BSP --- Jiunhau_Wang "[A86][Sensor][NA][Spec] use wq to process suspend/resume"
...
// ASUS_BSP +++ Jiunhau "[A86][Sensor][NA][Spec] Porting 9-axis sensor(Part II)"	
	result = inv_mpu_gyro_fsysc_init(client);
	if(result < 0) {
		printk(KERN_INFO "[MPU6500][probe] Gyro fsync init fail\n");
		goto out_no_free;
	}
// ASUS_BSP --- Jiunhau "[A86][Sensor][NA][Spec] Porting 9-axis sensor(Part II)"
	...
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
		result = -ENOSYS;
		printk(KERN_INFO "[MPU6500][probe] I2c function error\n");
		goto out_no_free;
	}
	indio_dev = iio_allocate_device(sizeof(*st));
	if (indio_dev == NULL) {
		printk(KERN_INFO "[MPU6500][probe] memory allocation failed\n");
		result =  -ENOMEM;
		goto out_no_free;
	}
	...
	g_MPU = st;
// ASUS_BSP +++ Jiunhau "[A86][Sensor][NA][Spec] support SR2"
	inv_change_orientation();
// ASUS_BSP --- Jiunhau "[A86][Sensor][NA][Spec] support SR2"
	st->plat_data = mpu6500_pdata;
	...
	if (result) {
		printk(KERN_INFO "[MPU6500][probe] Could not initialize device.\n");
		goto out_free;
	}
	result = st->set_power_state(st, false);
	if (result) {
		printk(KERN_INFO "[MPU6500][probe] %s could not be turned off.\n", st->hw->name);
		goto out_free;
	}
	...
	if (result) {
		printk(KERN_INFO "[MPU6500][probe] configure ring buffer fail\n");
		goto out_free;
	}
	result = iio_buffer_register(indio_dev, indio_dev->channels,
					indio_dev->num_channels);
	if (result) {
		printk(KERN_INFO "[MPU6500][probe] ring buffer register fail\n");
		goto out_unreg_ring;
	}

//ASUS_BSP +++ Jiunhau_Wang "[A86][Sensor][NA][Spec] Porting 9-axis sensor"
	// st->irq = client->irq;
	inv_mpu_gyro_irq_init(client, st);
//ASUS_BSP --- Jiunhau_Wang "[A86][Sensor][NA][Spec] Porting 9-axis sensor"
	result = inv_mpu_probe_trigger(indio_dev);
	if (result) {
		printk(KERN_INFO "[MPU6500][probe] trigger probe fail\n");
		goto out_remove_ring;
	}
//ASUS_BSP +++ Jiunhau_Wang "[A86][Sensor][NA][Spec] Porting 9-axis sensor"
	/* Tell the i2c counter, we have an IRQ */
	INV_I2C_SETIRQ(MPU, st->irq);
	// INV_I2C_SETIRQ(MPU, client->irq);
//ASUS_BSP --- Jiunhau_Wang "[A86][Sensor][NA][Spec] Porting 9-axis sensor"
	result = iio_device_register(indio_dev);
	if (result) {
		printk(KERN_INFO "[MPU6500][probe] IIO device register fail\n");
	...
	if (INV_MPU6050 == st->chip_type ||
		...
		if (result) {
			printk(KERN_INFO "[MPU6500][probe] create dmp sysfs failed\n");
			goto out_unreg_iio;
		}
	};
	...
	dev_info(&client->adapter->dev, "[MPU6500][probe] %s is ready to go!\n",
					indio_dev->name);
	printk("[MPU6500] %s ---\n",__FUNCTION__);
//ASUS_BSP +++ Jiunhau_Wang "[A86][Sensor][NA][ATD] support g-sensor and gyro status"
	mpu_status = 1;
...
//ASUS_BSP +++ Jiunhau_Wang "[A86][Sensor][NA][Other] support I2C stress test"
#ifdef CONFIG_I2C_STRESS_TEST
       i2c_add_test_case(st->client, "Sensor_MPU6500",ARRAY_AND_SIZE(gMPU6500_TestCaseInfo));
#endif //CONFIG_I2C_STRESS_TEST
//ASUS_BSP --- Jiunhau_Wang "[A86][Sensor][NA][Other] support I2C stress test"

//ASUS_BSP +++ Jiunhau_Wang "[A86][Sensor][NA][Spec] use wq to process suspend/resume"
	g_mpu_wq = create_singlethread_workqueue("mpu_wq");
	if (!g_mpu_wq) {
		printk(KERN_INFO "[MPU6500] create mpu wq fail\n");
	}
	INIT_DELAYED_WORK(&g_mpu_resume_work, mpu_resume_work);
//ASUS_BSP --- Jiunhau_Wang "[A86][Sensor][NA][Spec] use wq to process suspend/resume"
	return 0;
out_unreg_iio:
...
out_no_free:
	dev_err(&client->adapter->dev, "[MPU6500][probe]  %s failed %d\n", __func__, result);
...
{
//ASUS_BSP +++ Jiunhau_Wang "[A86][Sensor][NA][Spec] use wq to process suspend/resume"
	printk(KERN_INFO "[MPU6500] inv_mpu_resume\n");
	if(delayed_work_pending(&g_mpu_resume_work)){
		cancel_delayed_work_sync(&g_mpu_resume_work);
		printk(KERN_INFO "[MPU6500] cancel resume work\n");
	}
	queue_delayed_work(g_mpu_wq, &g_mpu_resume_work, msecs_to_jiffies(2));
	return 0;
//ASUS_BSP --- Jiunhau_Wang "[A86][Sensor][NA][Spec] use wq to process suspend/resume"
