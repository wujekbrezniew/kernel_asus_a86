...
static int a86_mdss_dsi_panel_power_on(struct mdss_panel_data *pdata, int enable)
{
    int ret;
	...
	if ((g_ASUS_hwID >= A86_SR1 && g_ASUS_hwID <= A86_SR2)) {
		 stb1_en = 65;
	 }
	 else if (g_ASUS_hwID >= A86_SR3) {
		 stb1_en = 54;
	 }
	 else if (g_ASUS_hwID == A86_EVB) {
		 stb1_en = 31;
	 }	
...
{
    if (mdss_panel_power) {
        a86_mdss_dsi_panel_power_on(g_mdss_pdata, 0);
    }
}
//ASUS_BSP:Louis "add panel power off sequence when kernel power off ---
...
static int mdss_dsi_off(struct mdss_panel_data *pdata)
{
    ...
    ret = a86_mdss_dsi_panel_power_on(pdata,0);
    //ASUS_BSP: Louis for display porting ---
	if (ret) {
...
#else
    ret = a86_mdss_dsi_panel_power_on(pdata, 1);
#endif
//ASUS_BSP: Louis for a86 miniporting ---
				...
				struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	if (pinfo->cont_splash_enabled) {
        ...
        rc = a86_mdss_dsi_panel_power_on(&(ctrl_pdata->panel_data), 1);
    	//ASUS_BSP: Louis for display miniporting --
