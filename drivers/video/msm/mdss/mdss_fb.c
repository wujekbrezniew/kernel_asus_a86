...
extern void nvt_hsic_set(struct asus_pad_hsic *hsic_info);
extern void renesas_set_ce( char *);
extern bool asus_padstation_exist_realtime(void);
extern char renesas_ce_param[33];
extern int g_CE_update;
static struct workqueue_struct *splendid_workqueue;
static struct work_struct update_splendid_work;
static void set_splendid_cmd_work(struct work_struct *work);

static char g_ce_params[33] = {0xCA, 0x00, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x08,
     0x20, 0x80, 0x80, 0x0A, 0x4A, 0x37, 0xA0, 0x55, 0xF8, 0x0C,
     0x0C, 0x20, 0x10, 0x3F, 0x3F, 0x00, 0x00, 0x10, 0x10, 0x3F,
     0x3F, 0x3F, 0x3F};

static int32_t renesas_hue_data = 0x0;
		...
		struct mdp_display_commit *disp_commit)
{
    ...
    if (!g_CE_update && (renesas_ce_param[1] != 0) && !asus_padstation_exist_realtime()) {
        queue_work(splendid_workqueue, &update_splendid_work);
        g_CE_update = 1;
    }
    //ASUS_BSP: Louis ---
	return ret;
...
static void set_splendid_cmd_work(struct work_struct *work)
{
    msleep(30); //wait 3 frames update
    printk("[Display][CE] Update CE Setting\n");
    renesas_set_ce(renesas_ce_param);
}
//ASU_BPS: Louis ---
...
static int __mdss_fb_perform_commit(struct msm_fb_data_type *mfd)
{
	...
	int ret = -ENOSYS;
#ifdef CONFIG_ASUS_HDMI
	struct mdss_panel_data *pdata;	//ASUS_BSP: Louis ++
#endif
...
#ifdef CONFIG_ASUS_HDMI 
	if (mfd->fbi->node == 0) {
		if ((display_commit_cnt > 0) && (display_commit_cnt % COMMIT_FRAMES_COUNT == 2)) {
			pdata = dev_get_platdata(&mfd->pdev->dev);
			if (mfd->bl_level_scaled != 0) {
				printk("[BL] Restore brightness(%d)\n", mfd->bl_level_scaled);
...
int pad_state = 0;//Mickey+++, add for showing logo when booting in pad
			 ...
			 unsigned long arg)
{
	...
	//Mickey+++, add for showing logo when booting in pad
    case MSMFB_RELEASE_ALL:
        ret = mdss_fb_release_all(info, true);
        break;
    case MSMFB_SET_PADSTATE:
        ret = copy_from_user(&pad_state, argp, sizeof(pad_state));
        if (ret)
            printk("SET pad_state failed (%d)\n", ret);
        break;
    case MSMFB_GET_PADSTATE:
        ret = copy_to_user(argp, &pad_state, sizeof(pad_state));
        if (ret)
            printk("GET pad_state failed (%d)\n", ret);
        break;
    case MSMFB_GET_BOOTINPAD:
        ret = copy_to_user(argp, &g_Pad_Bootup, sizeof(g_Pad_Bootup));
        if (ret)
            printk("GET BOOTINPAD failed (%d)\n", ret);
        break;
    //Mickey---

	//Louis for A86 CE io-ctrl +++
	case MSMFB_CE_SET:
		ret = copy_from_user(&g_ce_params, argp, sizeof(g_ce_params));
		if (ret) {
			printk(KERN_ERR "%s: copy from user failed \n",
					__func__);
		}
		g_ce_params[25] = renesas_hue_data;
		renesas_set_ce(g_ce_params);

		break;

	case MSMFB_PHONE_HUE_SET:
			ret = copy_from_user(&renesas_hue_data, argp, sizeof(renesas_hue_data));
			if (ret) {
				printk(KERN_ERR "%s: copy from user failed \n",
					__func__);
			}
		...
		//Louis for P05 HSIC ioctrl +++

	case MSMFB_PAD_HSIC_SET:
			ret = copy_from_user(&pad_hsic_data, argp, sizeof(pad_hsic_data));
			if (ret) {
				printk(KERN_ERR "%s: copy from user failed \n",
					__func__);
			}
			nvt_hsic_set(&pad_hsic_data);
...
int __init mdss_fb_init(void)
{
    ...
    //ASUS_BSP: Louis ++
    splendid_workqueue = create_singlethread_workqueue("splendid_wq");
    INIT_WORK(&update_splendid_work, set_splendid_cmd_work);
    //ASUS_BSP: Louis --
...
//ASUS_BSP: joe1_++: don't send MDSS_EVENT_SUSPEND to hdmi driver when entering early suspending because of hpd issue
	if (mfd->index == 1)
		return 0;
//ASUS_BSP: joe1_--: don't send MDSS_EVENT_SUSPEND to hdmi driver when entering early suspending because of hpd issue
