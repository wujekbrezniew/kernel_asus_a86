...
static long  pn544_dev_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case PN544_SET_PWR:
		if (arg == 2) {
			...
			gpio_set_value(pdata->firm_gpio, 1);
			msleep(20);
			printk("pdata->firm_gpio=%d\n", gpio_get_value(pdata->firm_gpio));
			...
			gpio_set_value(pdata->firm_gpio, 0);
			gpio_set_value(pn544_dev->ven_gpio, 1);
			msleep(100);
		} else  if (arg == 0) {
			/* power off */
...
#if 0
static void nfc_dt_get_download_gpio(struct device *dev, struct pn544_i2c_platform_data *pdata)
{
	int ret;
	//enum of_gpio_flags flags = OF_GPIO_ACTIVE_LOW;
	struct device_node *np = dev->of_node;

	pdata->firm_gpio = of_get_named_gpio(np,"download-gpio", 0);

        if(gpio_is_valid(pdata->firm_gpio)) {
		gpio_request(pdata->firm_gpio, "NFC_DOWNLOAD");
	}
...
#if 0
static int nfc_dt_parse_gpio_info(struct device *dev, struct pn544_i2c_platform_data *pdata)
{
	nfc_dt_get_download_gpio(dev, pdata);
	return 0;
}
#endif

static struct pn544_i2c_platform_data *nfc_populate_pdata(struct device *dev)
{
	int ret;
	struct device_node *np = dev->of_node;
...
static int pn544_probe( struct platform_device *pdev )
{
	int ret;
	int irq_num;
	...
	platform_data = nfc_populate_pdata(&pdev->dev);
	...
	pn544_dev->irq_gpio = NFC_GPIO_IRQ;
	pn544_dev->ven_gpio  = NFC_GPIO_EN;
	...
	*/
	...
	if (ret < 0) {
		printk("%s : not able to set nfc_download as output\n",__func__);
			goto err_exit;
	}
	#endif
	...
	#if 0
	config_nfc_download_gpio_pin(&pdev->dev);
	#endif
...
static int __init pn544_dev_init(void)
{
	...
	{
		printk("Remove Nfc driver in A86 Sr1 temporarily");
		return 0;
	}
	printk("Loading pn544 driver\n");
	return platform_driver_register(&pn544_driver);
}
...
//module_platform_driver(pn544_driver);
