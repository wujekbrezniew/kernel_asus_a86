...
#define ASUS_OC_ANTUTU
#ifdef ASUS_OC_ANTUTU
#define NORMAL_MAX_FREQ 2150400
static bool g_bANTUTU = false;
extern int e_g_ASUS_speed_bin;
extern int e_g_ASUS_pvs_bin;
#endif
...
show_one(freq_up_count, freq_up_count);
show_one(freq_down_count, freq_down_count);
show_one(power_saving, power_saving);
				    ...
				    const char *buf, size_t count)
{
	...
	pr_info("[DCVS] store powersave_bias (%d)", dbs_tuners_ins.powersave_bias);
	...
	if (!bypass) {
#ifdef ASUS_OC_ANTUTU
		if ((2 == e_g_ASUS_speed_bin && 2 == e_g_ASUS_pvs_bin) ||
			(2 == e_g_ASUS_speed_bin && 3 == e_g_ASUS_pvs_bin)) {
			g_bANTUTU = false;
		}
#endif
		if (reenable_timer) {
...
static ssize_t store_freq_up_count(struct kobject *a, struct attribute *b,
				   const char *buf, size_t count)
{
	unsigned int input;
	int ret;
	...
	if (ret != 1)
		return -EINVAL;
	dbs_tuners_ins.freq_up_count = input;
	return count;
}

static ssize_t store_freq_down_count(struct kobject *a, struct attribute *b,
				   const char *buf, size_t count)
{
	unsigned int input;
	int ret;
	...
	if (ret != 1)
		return -EINVAL;
	dbs_tuners_ins.freq_down_count = input;
	return count;
}

static ssize_t store_power_saving(struct kobject *a, struct attribute *b,
				   const char *buf, size_t count)
{
	unsigned input;
	int ret;
	...
	if (ret != 1 || (input != 0 && input != 1))
		return -EINVAL;
	dbs_tuners_ins.power_saving = input;

	if (dbs_tuners_ins.power_saving) {
		dbs_tuners_ins.powersave_bias = DEF_POWERSAVE_BIAS;
		pr_info("[DCVS] enable power saving mode (%d)\n", dbs_tuners_ins.powersave_bias);
	} else {
		dbs_tuners_ins.powersave_bias = 0;
		pr_info("[DCVS] disable power saving mode (%d)\n", dbs_tuners_ins.powersave_bias);
	}
...
define_one_global_rw(freq_up_count);
define_one_global_rw(freq_down_count);
define_one_global_rw(power_saving);
...
#ifdef ASUS_OC_ANTUTU
		if (p->cur == p->cpuinfo.max_freq && p->cpuinfo.max_freq != NORMAL_MAX_FREQ) {
			__cpufreq_driver_target(p, NORMAL_MAX_FREQ, CPUFREQ_RELATION_L);
		}
#endif
		return;
     }
#ifdef ASUS_OC_ANTUTU
	else if (!g_bANTUTU && p->cur == NORMAL_MAX_FREQ && p->max >= NORMAL_MAX_FREQ) {
		return;
	}

	if (!g_bANTUTU && freq == p->cpuinfo.max_freq) {
		freq = NORMAL_MAX_FREQ;
	}
#endif
...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
	...
	struct cpufreq_policy *policy;
	unsigned int j;
	bool ps = dbs_tuners_ins.power_saving;
	...
	if (max_load_freq > dbs_tuners_ins.up_threshold * policy->cur) {
		...
		if (ps) {
			if (dbs_tuners_ins.powersave_bias) {
				dbs_tuners_ins.sampling_count++;
				if (dbs_tuners_ins.sampling_count > dbs_tuners_ins.freq_up_count) {
					dbs_tuners_ins.powersave_bias=0;
					dbs_tuners_ins.sampling_count=0;
				}
			}
			else
				dbs_tuners_ins.sampling_count=0;
	...
	if (max_load_freq <
		...
		if (ps) {
			if (!dbs_tuners_ins.powersave_bias) {
					dbs_tuners_ins.sampling_count++;
					if (dbs_tuners_ins.sampling_count > dbs_tuners_ins.freq_down_count) {
						dbs_tuners_ins.powersave_bias=DEF_POWERSAVE_BIAS;
						dbs_tuners_ins.sampling_count = 0;
					}
			}
...
static void dbs_refresh_callback(struct work_struct *work)
{
	...
	if (policy->cur < target_freq) {
		 ...
		 */
#if defined(ASUS_OC_ANTUTU)
		if (!g_bANTUTU && target_freq == policy->cpuinfo.max_freq) {
			if (__cpufreq_driver_target(policy, NORMAL_MAX_FREQ,
						CPUFREQ_RELATION_L) >= 0)
				policy->cur = NORMAL_MAX_FREQ;
		} else {
...
static int dbs_sync_thread(void *data)
{
	...
	while (1) {
		...
		if (policy->cur < src_freq) {
			 ...
			 */
#ifdef ASUS_OC_ANTUTU
		if (!g_bANTUTU && src_freq == policy->cpuinfo.max_freq) {
			src_freq = NORMAL_MAX_FREQ;
		}
#endif
...
#ifdef ASUS_OC_ANTUTU
			if (!g_bANTUTU && policy->max == this_dbs_info->cur_policy->cpuinfo.max_freq) {
				__cpufreq_driver_target(this_dbs_info->cur_policy,
					NORMAL_MAX_FREQ, CPUFREQ_RELATION_H);
			} else
#endif
			__cpufreq_driver_target(this_dbs_info->cur_policy,
