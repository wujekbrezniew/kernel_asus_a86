...
extern struct qpnp_vadc_chip	*gauge_vadc_dev;
...
static ssize_t explain_dffs_string(char *OriginalMessages, unsigned long len)
{
	...
	if('V' == cmd)
	{
		...
		rc = qpnp_vadc_read(gauge_vadc_dev,VBAT_SNS, &result);
...
//Eason UpdateTIgauge +++
static ssize_t UpdateTIgauge_read_proc(char *page, char **start, off_t off, int count, 
            	int *eof, void *data)
{
	return 0;
}
...
{
	struct proc_dir_entry *UpdateTIgauge_proc_file = create_proc_entry("driver/UpdateTIgauge", 0666, NULL);

	if (UpdateTIgauge_proc_file) {
		UpdateTIgauge_proc_file->read_proc = UpdateTIgauge_read_proc;
		UpdateTIgauge_proc_file->write_proc = UpdateTIgauge_write_proc;
	}
    else {
