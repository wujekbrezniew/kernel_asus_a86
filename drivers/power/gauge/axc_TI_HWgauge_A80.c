...
#define FAKE_TEMP (25)
//ASUS BSP : Eason Chang check Df_version --------
static struct delayed_work delay_get_vadc_work;
...
struct qpnp_vadc_chip	*gauge_vadc_dev;
//Hank: update TIgauge global value---
...
//Hank: enter TIgauge Rom mode---
void delay_get_vadc(struct work_struct *work)
{
	int rc;
	gauge_vadc_dev = qpnp_get_vadc(&g_TIgauge_info->i2c_client->dev, "bms");
	if (IS_ERR(gauge_vadc_dev)) 
	{
		rc = PTR_ERR(gauge_vadc_dev);
		if (rc == -EPROBE_DEFER)
...
static void updatePcbThermal(void)
{
	...
	rc = qpnp_vadc_read(gauge_vadc_dev,LR_MUX4_PU2_AMUX_THM1, &result);
...
static int TIgauge_i2c_probe(struct i2c_client *client, const struct i2c_device_id *devid)
{
    ...
    printk("[BAT][GAU][TI]%s+++\n",__FUNCTION__);
    INIT_DELAYED_WORK(&delay_get_vadc_work,delay_get_vadc);
    schedule_delayed_work(&delay_get_vadc_work, 1*HZ);
    //ASUS BSP Eason_Chang TIgauge +++
   ...
   schedule_delayed_work(&update_TIgaugeGlobalValue_worker,round_jiffies_relative(1*HZ));
   //Hank: update global variable right now---
