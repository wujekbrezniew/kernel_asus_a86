...
{
	*curr = get_Curr_from_TIgauge();
 	*volt = get_Volt_from_TIgauge();
 	return;
}
...
//Hank read BatteryID+++
int32_t read_battery_id(void)
{
	int rc;
	struct qpnp_vadc_result result;

	rc = qpnp_vadc_read(gauge_vadc_dev,LR_MUX2_BAT_ID, &result);
	if (rc) {
	...
	g_BatteryID_value = result.adc_code;
...
int AXC_Gauge_A66_GetNextPollingInterval(struct AXI_Gauge *gauge)
{
	...
	//Hank A86 change update interval when Cap<=14% +++
