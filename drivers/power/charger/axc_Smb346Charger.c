...
static void smb346_otg_enable(bool switchOtg)
{
		...
		//Hank:only OTG I2c Control need do+++
		if(g_ASUS_hwID <= A86_SR1)
			smb346OtgEnableBit(switchOtg);// enable/disable vbus output by I2C
		//Hank:only OTG I2c Control need do---
		...
		if(g_ASUS_hwID == A86_EVB) {
			smb346_INOK_active_High(switchOtg);
		}
		//ASUS_BSP--- BennyCheng "add vbus output support by charger IC for usb OTG"
...
{
#ifndef ASUS_FACTORY_BUILD
	printk("enter setChgDrawCurrent!\n");
...
{
#ifndef ASUS_FACTORY_BUILD   
	printk("enter setChgDrawACTypeCurrent_withCheckAICL!\n");
	if(NORMAL_CURRENT_CHARGER_TYPE==gpCharger->type)
	{
		setChgLimitThermalRuleDrawCurrent(false);
	}else if( (3==g_thermal_limit)||(true==g_audio_limit) )
