...
{
	int bat_life = 0;
	//int var_drop_base=0; //Hank no use+++
	...
	if (drop_val > 0) //capacity drop
	{
		...
		if (interval > 0) 
		{
			...
			if(10==maxMah)
			{
				...
				*/
				pastTimeAvgCurr = cntPastAvgCurr(interval,filRealSusT,gCurr_TIgauge);
			...
			*/
			...
			//Hank:A86 slowly drop+++
			if( (g_ASUS_hwID >= A86_SR1)&&(nowCap<=OCV_PER_SPEEDUP_UPDATE_14P) )
			{
...
//Hank:A86 slowly drop---
	...
	//Eason: if BatLow keep 15 min, shutdown devices+++
	if ((g_ASUS_hwID >= A86_SR1)&& isBatLow && g_batLowLongTimeShut && (nowCap <= BATLOW_LONGTIME_SHUT_CAP) && (lastCap <= BATLOW_LONGTIME_SHUT_CAP)){
