...
#define I2C_TEST_ISPcamera_FAIL (-1)
struct msm_sensor_ctrl_t *ispcameras_ctrl;

static int TestIspcameraI2C (struct i2c_client *apClient)
{
       int lnResult = I2C_TEST_PASS;
       int err = 0;

       i2c_log_in_test_case("TestIspcameraI2C ++\n");
       err = msm_sensor_power_up(ispcameras_ctrl);
       //err = mt9m114_sensor_power_up(backs_ctrl);
       //err = msm_sensor_match_id(backs_ctrl);
       if(err < 0)     {
               i2c_log_in_test_case("Fail to power up Isp Camera\n");
               lnResult = I2C_TEST_ISPcamera_FAIL;
               return lnResult;
       }
       err = msm_sensor_power_down(ispcameras_ctrl);
       //err = mt9m114_sensor_power_down(backs_ctrl);
       if(err < 0)     {
               i2c_log_in_test_case("Fail to power down Isp Camera\n");
               lnResult = I2C_TEST_ISPcamera_FAIL;
       }
       i2c_log_in_test_case("TestIspcameraI2C --\n");
       return lnResult;
...
static struct i2c_test_case_info IspcameraTestCaseInfo[] =
{
     __I2C_STRESS_TEST_CASE_ATTR(TestIspcameraI2C),
};
#endif
...
//ASUS_BSP +++ LiJen "[A86][Camera][NA][Others]Camera mini porting"
	...
	s_ctrl->msm_sd.close_seq = MSM_SD_CLOSE_2ND_CATEGORY | 0x3;
	msm_sd_register(&s_ctrl->msm_sd);
	pr_err("surya %s:%d,%d\n", __func__, __LINE__,g_ASUS_hwID);
	if(g_ASUS_hwID < A91_SR1){
		pr_err("surya g_ASUS_hwID < A91_SR1--1390");
...
#ifdef CONFIG_I2C_STRESS_TEST
       if (0 == strncmp(client->name, "mt9m114", 7) )
       {
               pr_err("Camera add test case+\n");
               //pr_err("CONFIG_I2C_STRESS_TEST:%s",client->name);
               ispcameras_ctrl = s_ctrl;
               i2c_add_test_case(client, "camera_isp",ARRAY_AND_SIZE(IspcameraTestCaseInfo));
               pr_err("Camera add test case-\n");
       }
#endif
