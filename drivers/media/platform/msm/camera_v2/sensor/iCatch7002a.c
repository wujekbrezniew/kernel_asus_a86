			      ...
			      const struct sensor_reg table[])
{
	...
	while (next->addr != SENSOR_TABLE_END) {
		...
		if (next->addr == SEQ_WRITE_START) {
			...
			if (err) {
				pr_err("%s(%d): isensor_write_reg ret= 0x%x\n", __FUNCTION__, __LINE__, err);
				return err;
			}
		}
		next += 1;
...
int I2C_SPIFlashWrEnable(void)
{
	...
	ret = sensor_write_table(mt9m114_s_ctrl.sensor_i2c_client->client, I2C_SPIFlashWrEnable_seq);
...
void get_one_page_from_i7002a(int which_page, u8* pagebuf)
{
	...
	if (ret) {
		...
		pr_err("%s: get nothing. ret= %d", __FUNCTION__, ret);
		return;
	}
...
void enable_isp_interrupt(void){
    const unsigned int gpio_isp_int = 25; //LiJen: tmp
...
//ASUS_BSP --- Stimber "Implement the interface for calibration"
            ...
            case MSM_SENSOR_RES_10M:            //MODE_5
                ...
                sensor_write_reg(mt9m114_s_ctrl.sensor_i2c_client->client, 0x7106, 0x07);//preview resolution
                //sensor_write_reg(mt9m114_s_ctrl.sensor_i2c_client->client, 0x7120, 0x00);//swtich preview mode              
                data[0]=0x00; //for multiwrite reg(0x7120)
            ...
            case MSM_SENSOR_RES_HYBRID:     //MODE_6
                ...
                sensor_write_reg(mt9m114_s_ctrl.sensor_i2c_client->client, 0x7106, 0x09);//preview resolution
                //sensor_write_reg(mt9m114_s_ctrl.sensor_i2c_client->client, 0x7120, 0x00);//swtich preview mode
                data[0]=0x00; //for multiwrite reg(0x7120)
	...
	if (bSpeedUpPreview) { //ASUS_BSP YM Performance: to config stream early, when the previous captureStream is off, and now turn on preview stream only
	       ...
	       //multiwrite reg(0x7120), reg(0x7121)
	       //0x7120
	       //0x7121
	       data[1] = 0x21;       
	       data[2] = 0x01;
	       sensor_write_reg_bytes(mt9m114_s_ctrl.sensor_i2c_client->client, 0x7120, data, streamon_bytes);
	       pr_info("%s reg(0x7120)=0x%x\n",__func__,data[0]);
	       pr_info("%s reg(0x7121)=0x%x\n",__func__,data[2]);       
	       //sensor_write_reg(mt9m114_s_ctrl.sensor_i2c_client->client, 0x7121, 0x01);
	}
            	...
            	int *eof, void *data)
{
	...
	if(*eof == 0){
		...
		if(((ISPVersionNum==0xffffff)||(ISPVersionNum<BinVersionNum)) && (BinVersionNum!=0xffffff)) {
...
void iCatch_get_exif(struct exif_cfg *exif)
{
 ...
 //         (g_mi1040_power == true && g_cur_res ==MSM_SENSOR_RES_FULL) ||
          ((g_cur_res == MSM_SENSOR_RES_QTR || g_cur_res == MSM_SENSOR_RES_HYBRID )&& iCatch_exif_flow_control())) {
...
#endif
            //ASUS_BSP+++ jim3_lin "Add log while kernel got 1st frame after streamon"
            if( g_is1stFrame ){
                pr_info("%s: Got 1st frame",__func__);
                g_is1stFrame = false;
            }
            //ASUS_BSP--- jim3_lin "Add log while kernel got 1st frame after streamon"
...
//ASUS_BSP+++ jim3_lin "Pass G sensor data to ISP through i2c 15hz for CAF"
...
void iCatch_set_touch_AF(cam_focus_mode_type mode, int16_t coordinate_x, int16_t coordinate_y, int16_t rectangle_h, int16_t rectangle_w){
...
//ASUS_BSP +++ bill_chen "Implement image stabilization"
		case ASUS_CAM_INTF_PARM_EIS:
			...
			pr_err("EIS set to %d, %s auto night mode",g_is_eis_on,g_is_eis_on?"disable":"enable");
			if(g_is_eis_on)
...
void dump_isp_data(void)
{
    ...
    int dump_count[]=
    {
     ...
     for(i=0;i<6;i++)
      {
         printk("from addr 0x%x, the value is as following\n",dump_addr[i] );
         for(j=0;j<dump_count[i];j++)
         {
              sensor_read_reg(mt9m114_s_ctrl.sensor_i2c_client->client, dump_addr[i]+j, (u16 *)&i2c_get_value); 
              printk("0x%x ",i2c_get_value );
         }
         printk("\n");
      }
}
EXPORT_SYMBOL(dump_isp_data);
