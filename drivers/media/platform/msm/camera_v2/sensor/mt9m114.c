...
//ASUS_BSP +++ LiJen "[A86][Camera][NA][Others]Camera mini porting"
		...
		rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->
			...
			MSM_CAMERA_I2C_WORD_DATA);
#endif	
              g_cur_res = (int)cdata->cfg.setting;    //ASUS_BSP Stimber "Implement all sensor mode"
...
//ASUS_BSP +++ LiJen "[A86][Camera][NA][Others]Camera mini porting"
	//        if(g_pre_res != g_cur_res){
	           sensor_write_reg(mt9m114_s_ctrl.sensor_i2c_client->client, 0x7121, 0x00); 	//ASUS_BSP LiJen "[A86][Camera][NA][Others]Camera mini porting"
...
//ASUS_BSP--- YM only turn off stream, when the sensor is turned on	
	...
	case CFG_SET_START_STREAM:
		...
		rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->
			...
			MSM_CAMERA_I2C_WORD_DATA);
#endif	//ASUS_BSP --- LiJen "[A86][Camera][NA][Others]Camera mini porting"
        memset(&g_JpegExif,  0,  sizeof(struct exif_cfg)); //ASUS_BSP LiJen "[A86][Camera][NA][Others]Camera mini porting"
    	//ASUS_BSP +++ Stimber "Add sensor set mode interface"
//        if(g_pre_res != g_cur_res){
...
//ASUS_BSP +++ LiJen "[A86][Camera][NA][Others]Camera mini porting"
        ...
        case CFG_SET_CONTRAST: {
               ...
               pr_info("%s: Contrast Value is %d",__func__, con_lev);
             break;
        }
...
//ASUS_BSP--- YM only turn off stream, when the sensor is turned on	
    ...
    //enable isp interrupt 
     //gpio_direction_input(mt9m114_s_ctrl.sensordata->sensor_platform_info->isp_int);
	//rc = request_irq(MSM_GPIO_TO_INT(mt9m114_s_ctrl.sensordata->sensor_platform_info->isp_int), iCatch_irq_handler,
	//		IRQF_TRIGGER_RISING, "iCatch_int", 0);
	//gpio_direction_input(25);
	rc = request_irq(gpio_to_irq(25), iCatch_irq_handler,
			IRQF_TRIGGER_RISING, "iCatch_int", 0);
	            ...
	            unsigned long len, void *data)
{
	...
	if (messages == '\0') {
		...
		switch(messages){
			case '0':
				if(fled_on){
        				gpio_set_value(gpio_falsh, 0);
        				
        				pr_info("[Camera] gpio fled_driver_ent(%d)\n",gpio_get_value(gpio_falsh));
        				gpio_free(gpio_falsh);
			...
			case '1':
				if(!fled_on){
					rc = gpio_request(gpio_falsh, "mt9m114");
					if (rc) {
						pr_err("%s: [Camera] gpio FLED_DRIVER_ENT, rc(%d)fail\n",__func__, rc);
					}
						
					rc = gpio_direction_output(gpio_falsh, 1);
					if (rc != 0){
						pr_err("%s:[Camera] FLED enable failed\n", __func__);
					}	
                                   gpio_set_value(gpio_falsh, 1);
					pr_info("[Camera] gpio fled_driver_ent(%d)\n",gpio_get_value(gpio_falsh));
			...
			case '2':
				...
				//sensor_write_reg(mt9m114_s_ctrl.sensor_i2c_client->client, 0x7106, 0x06);
				//sensor_write_reg(mt9m114_s_ctrl.sensor_i2c_client->client, 0x7120, 0x00); //swtich preview mode
