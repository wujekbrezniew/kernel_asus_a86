...
#define OV2720ASUS_SENSOR_NAME "ov2720asus"
#define PLATFORM_DRIVER_NAME "msm_camera_ov2720asus"
#define ov2720asus_obj ov2720asus_##obj
...
/* Sysctl registers */
#define OV2720ASUS_COMMAND_REGISTER                0x0080
#define OV2720ASUS_COMMAND_REGISTER_APPLY_PATCH    (1 << 0)
#define OV2720ASUS_COMMAND_REGISTER_SET_STATE      (1 << 1)
#define OV2720ASUS_COMMAND_REGISTER_REFRESH        (1 << 2)
#define OV2720ASUS_COMMAND_REGISTER_WAIT_FOR_EVENT (1 << 3)
#define OV2720ASUS_COMMAND_REGISTER_OK             (1 << 15)

DEFINE_MSM_MUTEX(ov2720asus_mut);
...
static unsigned char g_ov2720asus_power = false;
//ASUS_BSP --- LiJen "[A86][Camera][NA][Others]Camera mini porting"

static struct msm_sensor_power_setting ov2720asus_power_setting[] = {
//ASUS_BSP +++ LiJen "[A86][Camera][NA][Others]Camera mini porting"
	{
...
//ASUS_BSP +++ LiJen "[A86][Camera][NA][Others]Camera mini porting"
static struct msm_sensor_power_setting ov2720asus_power_setting_isp[] = {
	{
		.seq_type = SENSOR_GPIO,
		.seq_val = SENSOR_GPIO_ISP_RESET,
		.config_val = GPIO_OUT_LOW,
...
static struct msm_camera_i2c_reg_conf ov2720asus_720p_settings[] = {
	{0xdc00, 0x50, MSM_CAMERA_I2C_BYTE_DATA, MSM_CAMERA_I2C_CMD_WRITE},
	{OV2720ASUS_COMMAND_REGISTER, OV2720ASUS_COMMAND_REGISTER_SET_STATE,
		MSM_CAMERA_I2C_UNSET_WORD_MASK, MSM_CAMERA_I2C_CMD_POLL},
	{OV2720ASUS_COMMAND_REGISTER, (OV2720ASUS_COMMAND_REGISTER_OK |
		OV2720ASUS_COMMAND_REGISTER_SET_STATE), MSM_CAMERA_I2C_WORD_DATA,
...
#if 0	//ASUS_BSP +++ LiJen "[A86][Camera][NA][Others]Camera mini porting"
static struct msm_camera_i2c_reg_conf ov2720asus_recommend_settings[] = {
...
static struct v4l2_subdev_info ov2720asus_subdev_info[] = {
	{
...
#if 0	//ASUS_BSP +++ LiJen "[A86][Camera][NA][Others]Camera mini porting"
static struct msm_camera_i2c_reg_conf ov2720asus_config_change_settings[] = {
...
static const struct i2c_device_id ov2720asus_i2c_id[] = {
	{OV2720ASUS_SENSOR_NAME, (kernel_ulong_t)&ov2720asus_s_ctrl},
	{ }
};
//ASUS_BSP +++ LiJen "[A86][Camera][NA][Others]Camera mini porting"
static int32_t msm_ov2720asus_i2c_probe(struct i2c_client *client,
	const struct i2c_device_id *id)
{
	return msm_sensor_i2c_probe(client, id, &ov2720asus_s_ctrl);
}
//ASUS_BSP --- LiJen "[A86][Camera][NA][Others]Camera mini porting"
static struct i2c_driver ov2720asus_i2c_driver = {
	.id_table = ov2720asus_i2c_id,
	.probe  = msm_ov2720asus_i2c_probe,
	.driver = {
		.name = OV2720ASUS_SENSOR_NAME,
	},
};

static struct msm_camera_i2c_client ov2720asus_sensor_i2c_client = {
	.addr_type = MSM_CAMERA_I2C_WORD_ADDR,
};

static const struct of_device_id ov2720asus_dt_match[] = {
	{.compatible = "qcom,ov2720asus", .data = &ov2720asus_s_ctrl},
	{}
};

MODULE_DEVICE_TABLE(of, ov2720asus_dt_match);

static struct platform_driver ov2720asus_platform_driver = {
	.driver = {
		.name = "qcom,ov2720asus",
		.owner = THIS_MODULE,
		.of_match_table = ov2720asus_dt_match,
	},
};

static int32_t ov2720asus_platform_probe(struct platform_device *pdev)
{
	int32_t rc;
	const struct of_device_id *match;
	match = of_match_device(ov2720asus_dt_match, &pdev->dev);
	rc = msm_sensor_platform_probe(pdev, match->data);
//ASUS_BSP +++ LiJen "[A86][Camera][NA][Others]Camera mini porting"
	if ( rc != 0 )
		pr_err("msm_sensor_platform_probe fail, Error : %d\n",rc); 
    
       rc = i2c_add_driver(&ov2720asus_i2c_driver);
	if ( rc != 0 )
...
extern void create_ov2720asus_proc_file(void); //ASUS_BSP LiJen "[A86][Camera][NA][Others]Camera mini porting"
static int __init ov2720asus_init_module(void)
{
	int32_t rc;
	pr_info("%s:%d\n", __func__, __LINE__);
...
//ASUS_BSP --- LiJen "[A86][Camera][NA][Others]Camera mini porting"
	...
	rc = platform_driver_probe(&ov2720asus_platform_driver,
		ov2720asus_platform_probe);
	if (!rc)
		return rc;
	pr_err("%s:%d rc %d\n", __func__, __LINE__, rc);
	return i2c_add_driver(&ov2720asus_i2c_driver);
}

static void __exit ov2720asus_exit_module(void)
{
	pr_info("%s:%d\n", __func__, __LINE__);
	if (ov2720asus_s_ctrl.pdev) {
		msm_sensor_free_sensor_data(&ov2720asus_s_ctrl);
		platform_driver_unregister(&ov2720asus_platform_driver);
              i2c_del_driver(&ov2720asus_i2c_driver); //ASUS_BSP LiJen "[A86][Camera][NA][Others]Camera mini porting"
	} else
		i2c_del_driver(&ov2720asus_i2c_driver);
	return;
}

int32_t ov2720asus_sensor_config(struct msm_sensor_ctrl_t *s_ctrl,
	void __user *argp)
{
...
//ASUS_BSP +++ LiJen "[A86][Camera][NA][Others]Camera mini porting"  
       if(cdata->cfgtype == CFG_SET_SLAVE_INFO ||
            ...
            //don't need to check power up before do task
       }else{
        //need to check power up before do task
           if(false == g_ov2720asus_power && CFG_POWER_UP != cdata->cfgtype){
...
//ASUS_BSP --- LiJen "[A86][Camera][NA][Others]Camera mini porting" 
	...
	case CFG_SET_INIT_SETTING:
		...
		ov2720asus_s_ctrl.sensor_i2c_client->client->addr=0x3c<<1;
		sensor_write_reg(ov2720asus_s_ctrl.sensor_i2c_client->client, 0x1011, 0x01);//cpu reset
		sensor_write_reg(ov2720asus_s_ctrl.sensor_i2c_client->client, 0x001C, 0x08);
		sensor_write_reg(ov2720asus_s_ctrl.sensor_i2c_client->client, 0x001C, 0x00);
		sensor_write_reg(ov2720asus_s_ctrl.sensor_i2c_client->client, 0x1010, 0x02);
		sensor_write_reg(ov2720asus_s_ctrl.sensor_i2c_client->client, 0x1010, 0x00);
		sensor_write_reg(ov2720asus_s_ctrl.sensor_i2c_client->client, 0x1306, 0x01);//front camera
		sensor_write_reg(ov2720asus_s_ctrl.sensor_i2c_client->client, 0x1011, 0x00);
		msleep(10);
#if 0	//ASUS_BSP +++ LiJen "[A86][Camera][NA][Others]Camera mini porting"  
		rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->
			i2c_write_conf_tbl(
			s_ctrl->sensor_i2c_client, ov2720asus_recommend_settings,
			ARRAY_SIZE(ov2720asus_recommend_settings),
			MSM_CAMERA_I2C_WORD_DATA);
		rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->
			i2c_write_conf_tbl(
			s_ctrl->sensor_i2c_client,
			ov2720asus_config_change_settings,
			ARRAY_SIZE(ov2720asus_config_change_settings),
			MSM_CAMERA_I2C_WORD_DATA);
#endif 	//ASUS_BSP --- LiJen "[A86][Camera][NA][Others]Camera mini porting"   
		break;
	case CFG_SET_RESOLUTION:
		...
		rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->
			i2c_write_conf_tbl(
			s_ctrl->sensor_i2c_client, ov2720asus_720p_settings,
			ARRAY_SIZE(ov2720asus_720p_settings),
			MSM_CAMERA_I2C_WORD_DATA);
#endif	//ASUS_BSP --- LiJen "[A86][Camera][NA][Others]Camera mini porting"  
		break;
	case CFG_SET_STOP_STREAM:
               if(iCatch_first_open){
                    sensor_write_reg(ov2720asus_s_ctrl.sensor_i2c_client->client, 0x7121, 0x00); 	//ASUS_BSP LiJen "[A86][Camera][NA][Others]Camera mini porting"
               }
		break;
	case CFG_SET_START_STREAM:
              if(iCatch_first_open){
		    pr_info("Switch preview mode\n");
		    sensor_write_reg(ov2720asus_s_ctrl.sensor_i2c_client->client, 0x7106, 0x0D);//preview mode
		    sensor_write_reg(ov2720asus_s_ctrl.sensor_i2c_client->client, 0x7120, 0x00);        
		    sensor_write_reg(ov2720asus_s_ctrl.sensor_i2c_client->client, 0x7121, 0x01);	 //ASUS_BSP LiJen "[A86][Camera][NA][Others]Camera mini porting"
		    //wait_for_AWB_ready();
		...
		if (0) { 
			u16 idH=0, idL=0;
			msleep(100);
			sensor_read_reg(ov2720asus_s_ctrl.sensor_i2c_client->client, 0x72c8, &idL);
			sensor_read_reg(ov2720asus_s_ctrl.sensor_i2c_client->client, 0x72c9, &idH);
			pr_err("Randy sensor id = %x %x\n", idH, idL);
		}
#if 0	//ASUS_BSP +++ LiJen "[A86][Camera][NA][Others]Camera mini porting"
		rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->
			i2c_write_conf_tbl(
...
//ASUS_BSP +++ LiJen "[A86][Camera][NA][Others]Camera mini porting"
int32_t ov2720asus_sensor_power_up(struct msm_sensor_ctrl_t *s_ctrl)
{
    if(g_ov2720asus_power == true){
        pr_info("%s power has enabled\n", __func__);
        return -0;	            
    } 

    if(true != g_ISPbootup){
        s_ctrl->power_setting_array.power_setting = ov2720asus_power_setting;
        s_ctrl->power_setting_array.size = ARRAY_SIZE(ov2720asus_power_setting);        
    }else{
        s_ctrl->power_setting_array.power_setting = ov2720asus_power_setting_isp;
        s_ctrl->power_setting_array.size = ARRAY_SIZE(ov2720asus_power_setting_isp);
    }
...
int32_t ov2720asus_sensor_power_down(struct msm_sensor_ctrl_t *s_ctrl)
{
    if(false == g_ov2720asus_power){
    ...
    g_ov2720asus_power = false;
    return 0;
}
//ASUS_BSP --- LiJen "[A86][Camera][NA][Others]Camera mini porting"

static struct msm_sensor_fn_t ov2720asus_sensor_func_tbl = {
	.sensor_config = ov2720asus_sensor_config,
	.sensor_power_up = ov2720asus_sensor_power_up, //ASUS_BSP LiJen "[A86][Camera][NA][Others]Camera mini porting"
	.sensor_power_down = ov2720asus_sensor_power_down, //ASUS_BSP LiJen "[A86][Camera][NA][Others]Camera mini porting"
	.sensor_match_id = msm_sensor_match_id,
};

struct msm_sensor_ctrl_t ov2720asus_s_ctrl = { //ASUS_BSP LiJen "[A86][Camera][NA][Others]Camera mini porting"
	.sensor_i2c_client = &ov2720asus_sensor_i2c_client,
	.power_setting_array.power_setting = ov2720asus_power_setting,
	.power_setting_array.size = ARRAY_SIZE(ov2720asus_power_setting),
	.msm_sensor_mutex = &ov2720asus_mut,
	.sensor_v4l2_subdev_info = ov2720asus_subdev_info,
	.sensor_v4l2_subdev_info_size = ARRAY_SIZE(ov2720asus_subdev_info),
	.func_tbl = &ov2720asus_sensor_func_tbl,
};
//ASUS_BSP --- LiJen "[A86][Camera][NA][Others]Camera mini porting"

module_init(ov2720asus_init_module);
module_exit(ov2720asus_exit_module);
