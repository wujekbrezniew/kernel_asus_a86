...
//ASUS_BSP +++ Jiunhau_Wang "[A86][Sensor][NA][Spec] Porting haptic sensor"
...
//static int drv2605_irq_gpio = 0;
//Rice: added to vibrate after phone inserted into pad.
...
static struct drv2605_platform_data drv2605_plat_data = {
	...
	.actuator = {
               .device_type = ERM,
               .g_effect_bank = LIBRARY_D,    //according to actuator spec
               .loop = CLOSE_LOOP,     //according to actuator spec
               .rated_vol = 0x8e,      //according to actuator spec
               .over_drive_vol = 0xA8, //according to actuator spec
...
static void drv260x_change_mode(struct i2c_client *client, char mode)
{
	...
	if(((pDrv2605data->mode == MODE_STANDBY)||(pDrv2605data->mode == MODE_PATTERN_RTP_OFF))
		&&(tmp[1] != MODE_STANDBY)){
...
{
	//struct Haptics	*pvibdata = container_of(dev, struct Haptics, to_dev); // Jiunhau
...
//ASUS_BSP +++ -
static ssize_t read_drv2605_status(struct device *dev, struct device_attribute *devattr, char *buf)
{	
	return sprintf(buf, "%d\n", haptics_status);
}

DEVICE_ATTR(drv2605_status, S_IRUGO, read_drv2605_status, NULL);

static struct attribute *drv2605_attr[] = {
	&dev_attr_drv2605_status.attr,
	NULL,
};
...
static int dev_auto_calibrate(struct i2c_client* client, struct drv2605_data *pDrv2605data)
{
	...
	if(pDrv2605data->device_id != DRV2605){
...
{
	if(!g_drv2605_probe_ok){
		printk("[%s] vibrator not use i2c !!!\n", __func__);
		return 0;
	}
	...
	switch (event)
    {
        case P01_ADD:
        {
            ...
            vibrator_enable(&vibdata.to_dev,500);
            return NOTIFY_DONE;
        }
...
/*
            hrtimer_cancel(&g_vib->vib_timer);
            cancel_work_sync(&g_vib->work);
            pm8xxx_vib_set(g_vib, 0);
*/
            vibrator_enable(&vibdata.to_dev,0);
        }
...
{
	struct drv2605_data *pDrv2605data = NULL;
	struct drv2605_platform_data *pDrv2605Platdata = NULL;
	
	int err = 0;
	int status = 0;
	
	if (g_ASUS_hwID > A86_SR4) 
	{	
		g_drv2605_probe_ok = 0; //match qpnp-vibrator.c 	
		return 0;
	}
...
//ASUS_BSP +++ Jiunhau_Wang "[A86][Sensor][NA][Spec] Porting haptic sensor"
	printk("[DRV2605] %s +++\n",__FUNCTION__);
	pDrv2605Platdata = &drv2605_plat_data;

//ASUS_BSP --- Jiunhau_Wang "[A86][Sensor][NA][Spec] Porting haptic sensor"

	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C))
	{
		printk(KERN_INFO"[DRV2605] I2C check failed\n");
		return -ENODEV;
	}
	...
	if(!pDrv2605data){
		err = -ENOMEM;
		printk(KERN_INFO"[DRV2605]-ENOMEM error\n");		
	...
	if(pDrv2605data->device_id != DRV2605){
		printk("%s, status(0x%x),device_id(%d) fail,drv2605 id=%d\n",
			__FUNCTION__, status, pDrv2605data->device_id, DRV2605);
		goto exit_gpio_request_failed2;
	}
    ...
    notify_register_microp_notifier(&mp_notifier, "haptics");
	//Rice: added to vibrate after phone inserted into pad.
	
    g_drv2605_probe_ok = 1;
...
static int drv260x_remove(struct i2c_client* client)
{
	...
	if (g_ASUS_hwID > A86_SR4) 
	{	
		printk("[%s] Vibrator not use i2c devices", __func__); //match qpnp-vibrator.c 	
		return 0;
	}

	pDrv2605Data = i2c_get_clientdata(client);
	...
	notify_unregister_microp_notifier(&mp_notifier, "haptics");
	//Rice: added to vibrate after phone inserted into pad.
    return 0;
}
