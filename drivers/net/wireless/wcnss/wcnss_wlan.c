...
//ASUS_BSP+++ "for wlan, BT antenna switch with the phone/pad mode"
/*------------------------------*/
...
//ASUS_BSP+++ "for CTS test in pad mod with Y-cable (20130322)"
#include <linux/fs.h>
...
static int g_wlan_driver_loaded = 0;
static int g_bt_driver_enable = 0;
static int g_padfone_plug_in = 0;

static char g_rf_switch_status[10];
static int g_PMIC_GPIO_WL_SW_1 = 0;
static int g_PMIC_GPIO_WL_SW_2 = 0;

static int g_ANT_GPIO_114 = 0;
static int g_ANT_GPIO_115 = 0;
static int g_ANT_GPIO_124 = 0;
static int g_ANT_GPIO_127 = 0;
static int g_ANT_GPIO_125 = 0;  /* Only for A86_SR2 */
/*--------------------------------------------------*/
//ASUS_BSP--- "for wlan, BT antenna switch with the phone/pad mode"

//ASUS_BSP+++ "for CTS test in pad mod with Y-cable (20130322)"
static int has_CTS_run = 0;
module_param(has_CTS_run, int, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
//ASUS_BSP--- "for CTS test in pad mod with Y-cable (20130322)"
...
struct mutex g_vreg_lock;
extern void wlan_ant_vregs_on(struct device *dev);
extern void wlan_ant_vregs_off(void);
...
//ASUS_BSP+++ "for wlan, BT antenna switch with the phone/pad mode"
/*--------------------------------------------------------------------------------*/
int wcnss_Wlan_ON_OFF(int ison);
int wcnss_BT_ON_OFF(int ison);
int wcnss_PadFone_IN_OUT(int isin);
void wcnss_Check_antenna(void);
void wcnss_set_antenna_OFF(void);
void wcnss_set_antenna_PHONE(void);
void wcnss_set_antenna_PADMAIN(void);
void wcnss_set_antenna_PADWIFI(void);
void wcnss_set_antenna_PHONE_Diversity( void );
static int wcnss_rf_switch_read_proc(char *page, char **start, off_t off, int count, int *eof, void *data);
static int wcnss_rf_switch_write_proc(struct file *file, const char __user *buffer, unsigned long count, void *data);
/*--------------------------------------------------------------------------------*/
//ASUS_BSP--- "for wlan, BT antenna switch with the phone/pad mode"


//ASUS_BSP+++ "for CTS test in pad mod with Y-cable (20130322)"
int wcnss_Check_CTS_Run(void)
{
    mm_segment_t oldfs;
    struct file *fp=NULL;
    ...
    fp = filp_open("/data/.tmp/has_CTS_run", O_RDONLY, 0);
    if( IS_ERR(fp) ) {
        printk("[wcnss]: filp_open fail.\n");
    }
    else {
        has_CTS_run = 1;
        printk("[wcnss]: filp_open OK.\n");
        filp_close(fp, NULL);
    }
...
static void wcnss_smd_notify_event(void *data, unsigned int event)
{
	...
	case SMD_EVENT_OPEN:
		pr_info("[wcnss]: SMD_EVENT_OPEN, opening WCNSS SMD channel :%s",
				WCNSS_CTRL_CHANNEL);
	...
	case SMD_EVENT_CLOSE:
		pr_info("[wcnss]: SMD_EVENT_CLOSE, closing WCNSS SMD channel :%s",
				WCNSS_CTRL_CHANNEL);
...
static void wcnss_post_bootup(struct work_struct *work)
{
	...
	pr_info("[wcnss]: %s: Cancel APPS vote for Iris & WCNSS\n", __func__);
...
wcnss_wlan_ctrl_probe(struct platform_device *pdev)
{
	...
	pr_info("[wcnss]: wcnss_wlan_ctrl_probe, SMD ctrl channel up.\n");
...
wcnss_trigger_config(struct platform_device *pdev)
{
	...
	if (wcnss_hardware_type() == WCNSS_RIVA_HW) {
		...
		pr_info("[wcnss]: RIVA, MSM_RIVA_CCU_BASE=0x03200800.\n");
		...
		pr_info("[wcnss]: PRONTO, MSM_PRONTO_A2XB_BASE=0xfb100400.\n");
	}
...
int wcnss_Wlan_ON_OFF(int ison)
{
    /*wlan driver is ON*/
    if( ison ) {
	    g_wlan_driver_loaded = 1;
        printk("[wcnss]: g_wlan_driver_loaded=1.\n");

        //ASUS_BSP+++ "for CTS test in pad mod with Y-cable (20130322)"
        if( has_CTS_run == 1 ) {
            wcnss_set_antenna_PHONE();
            printk("[wcnss]: wcnss_set_antenna_PHONE().\n");
            return g_wlan_driver_loaded;
        }
        //ASUS_BSP--- "for CTS test in pad mod with Y-cable (20130322)"
    }
    else { /*wlan driver is OFF*/
        g_wlan_driver_loaded = 0;
        printk("[wcnss]: g_wlan_driver_loaded=0.\n");
...
EXPORT_SYMBOL(wcnss_Wlan_ON_OFF);
...
int wcnss_BT_ON_OFF(int ison)
{
    /*BT driver is ON*/
    if( ison ) {
        g_bt_driver_enable = 1;
        printk("[wcnss]: g_bt_driver_enable=1.\n");

        //ASUS_BSP+++ "for CTS test in pad mod with Y-cable (20130322)"
        if( has_CTS_run == 1 ) {
            wcnss_set_antenna_PHONE();
            printk("[wcnss]: wcnss_set_antenna_PHONE().\n");
            return g_bt_driver_enable;
        }
        //ASUS_BSP--- "for CTS test in pad mod with Y-cable (20130322)"
    }
    else { /*BT driver is OFF*/
        g_bt_driver_enable = 0;
        printk("[wcnss]: g_bt_driver_enable=0.\n");

        //ASUS_BSP+++ "for CTS test in pad mod with Y-cable (20130322)"
        if( has_CTS_run == 1 ) {
            printk("[wcnss]: no switch antenna.\n");
            return g_bt_driver_enable;
        }
        //ASUS_BSP--- "for CTS test in pad mod with Y-cable (20130322)"
    }
...
int wcnss_PadFone_IN_OUT(int isin)
{
    ...
    printk("[wcnss]: g_padfone_plug_in=%d.\n", g_padfone_plug_in);
    wcnss_Check_antenna();

    return g_padfone_plug_in;
}
EXPORT_SYMBOL(wcnss_PadFone_IN_OUT);


void wcnss_Check_antenna( void )
{
    if( g_padfone_plug_in == 1 ) {
        if( (g_wlan_driver_loaded == 1) || (g_bt_driver_enable == 1) ){
            wcnss_set_antenna_PADWIFI();
        }
        else {
            wcnss_set_antenna_PADMAIN();
        }
    }
    else {
        if( (g_wlan_driver_loaded == 1) || (g_bt_driver_enable == 1) ){
...
void wcnss_set_antenna_OFF( void )
{
    printk("[wcnss]: wcnss_set_antenna_OFF.\n");

    if( (g_PMIC_GPIO_WL_SW_1 > 0) && (g_PMIC_GPIO_WL_SW_2 > 0) ){
        if( g_ASUS_hwID >= A86_SR3 ) {
            gpio_direction_output(g_PMIC_GPIO_WL_SW_1, 0);
            gpio_direction_output(g_PMIC_GPIO_WL_SW_2, 0);
        }
    }
    else {
        printk("[wcnss]: wcnss_set_antenna_OFF, WL_SW GPIO wrong.\n");
    }

    if( (g_ANT_GPIO_114 > 0) && (g_ANT_GPIO_115 > 0) && (g_ANT_GPIO_124 > 0) && (g_ANT_GPIO_127 > 0) ) {
        gpio_direction_output(g_ANT_GPIO_114, 1);
        gpio_direction_output(g_ANT_GPIO_115, 0);
        gpio_direction_output(g_ANT_GPIO_124, 0);
        gpio_direction_output(g_ANT_GPIO_127, 1);
    }
    else {
        printk("[wcnss]: wcnss_set_antenna_OFF, RF_SW GPIO wrong.\n");
    }

    if( g_ASUS_hwID == A86_SR2 ) {
        if( g_ANT_GPIO_125 > 0 ) {
            gpio_direction_output(g_ANT_GPIO_125, 0);
        }
        else {
            printk("[wcnss]: wcnss_set_antenna_OFF, RF_SW GPIO wrong (GPIO_125).\n");
        }
    }

    strcpy(g_rf_switch_status, "all_off");
...
void wcnss_set_antenna_PHONE( void )
{
    ...
    if( !penv ) {
        printk("[wcnss]: wcnss_set_antenna_PHONE, (penv=NULL).\n");
        return;
    }

    dev = &penv->pdev->dev;

    wlan_ant_vregs_on(dev);

    printk("[wcnss]: wcnss_set_antenna_PHONE.\n");

    if( (g_PMIC_GPIO_WL_SW_1 > 0) && (g_PMIC_GPIO_WL_SW_2 > 0) ) {
        if(g_ASUS_hwID >= A86_SR3) {
            gpio_direction_output(g_PMIC_GPIO_WL_SW_1, 1);
            gpio_direction_output(g_PMIC_GPIO_WL_SW_2, 1);
        }
        else{
            gpio_direction_output(g_PMIC_GPIO_WL_SW_1, 1);
            gpio_direction_output(g_PMIC_GPIO_WL_SW_2, 0);
        ...
        printk("[wcnss]: wcnss_set_antenna_PHONE, WL_SW GPIO wrong.\n");
    }
    ...
    if( (g_ANT_GPIO_114 > 0) && (g_ANT_GPIO_115 > 0) && (g_ANT_GPIO_124 > 0) && (g_ANT_GPIO_127 > 0) ) {
        ...
        printk("[wcnss]: wcnss_set_antenna_PHONE, RF_SW GPIO wrong.\n");
    }
    ...
    if( g_ASUS_hwID == A86_SR2 ) {
        if( g_ANT_GPIO_125 > 0 ) {
            ...
            printk("[wcnss]: wcnss_set_antenna_PHONE, RF_SW GPIO wrong (GPIO_125).\n");
        }
    }
...
void wcnss_set_antenna_PADMAIN( void )
{
    printk("[wcnss]: wcnss_set_antenna_PADMAIN.\n");
    ...
    if( (g_PMIC_GPIO_WL_SW_1 > 0) && (g_PMIC_GPIO_WL_SW_2 > 0) ){
        ...
        printk("[wcnss]: wcnss_set_antenna_PADMAIN, WL_SW GPIO wrong.\n");
    }

    if( (g_ANT_GPIO_114 > 0) && (g_ANT_GPIO_115 > 0) && (g_ANT_GPIO_124 > 0) && (g_ANT_GPIO_127 > 0) ) {
        gpio_direction_output(g_ANT_GPIO_114, 1);
        gpio_direction_output(g_ANT_GPIO_115, 1);
        gpio_direction_output(g_ANT_GPIO_124, 1);
        gpio_direction_output(g_ANT_GPIO_127, 1);
    }
    else {
        printk("[wcnss]: wcnss_set_antenna_PADMAIN, RF_SW GPIO wrong.\n");
    }
    ...
    if( g_ASUS_hwID == A86_SR2 ) {
        if( g_ANT_GPIO_125 > 0 ) {
            ...
            printk("[wcnss]: wcnss_set_antenna_PADMAIN, RF_SW GPIO wrong (GPIO_125).\n");
        }
    }
...
void wcnss_set_antenna_PADWIFI( void )
{
    ...
    if( !penv ) {
        printk("[wcnss]: wcnss_set_antenna_PADWIFI, (penv=NULL).\n");
        return;
    }
    ...
    printk("[wcnss]: wcnss_set_antenna_PADWIFI.\n");
    ...
    if( (g_PMIC_GPIO_WL_SW_1 > 0) && (g_PMIC_GPIO_WL_SW_2 > 0) ){
        ...
        printk("[wcnss]: wcnss_set_antenna_PADWIFI, WL_SW GPIO wrong.\n");
    }
    ...
    if( (g_ANT_GPIO_114 > 0) && (g_ANT_GPIO_115 > 0) && (g_ANT_GPIO_124 > 0) && (g_ANT_GPIO_127 > 0) ) {
        ...
        printk("[wcnss]: wcnss_set_antenna_PADWIFI, RF_SW GPIO wrong.\n");
    }
    ...
    if( g_ASUS_hwID == A86_SR2 ) {
        if( g_ANT_GPIO_125 > 0 ) {
            ...
            printk("[wcnss]: wcnss_set_antenna_PADWIFI, RF_SW GPIO wrong (GPIO_125).\n");
        }
    }

	strcpy(g_rf_switch_status, "pad_wifi");
}
...
void wcnss_set_antenna_PHONE_Diversity( void )
{
    ...
    if( !penv ) {
        printk("[wcnss]: wcnss_set_antenna_PHONE_Diversity, (penv=NULL).\n");
        return;
    }
    ...
    printk("[wcnss]: wcnss_set_antenna_PHONE_Diversity.\n");
    ...
    if( (g_PMIC_GPIO_WL_SW_1 > 0) && (g_PMIC_GPIO_WL_SW_2 > 0) ) {
        ...
        printk("[wcnss]: wcnss_set_antenna_PHONE_Diversity, WL_SW GPIO wrong.\n");
    }
    ...
    if( (g_ANT_GPIO_114 > 0) && (g_ANT_GPIO_115 > 0) && (g_ANT_GPIO_124 > 0) && (g_ANT_GPIO_127 > 0) ) {
        ...
        gpio_direction_output(g_ANT_GPIO_115, 0);
        gpio_direction_output(g_ANT_GPIO_124, 0);
        gpio_direction_output(g_ANT_GPIO_127, 0);
    }
    else {
        printk("[wcnss]: wcnss_set_antenna_PHONE_Diversity, RF_SW GPIO wrong.\n");
    }
    ...
    if( g_ASUS_hwID == A86_SR2 ) {
        if( g_ANT_GPIO_125 > 0 ) {
            ...
            printk("[wcnss]: wcnss_set_antenna_PHONE_Diversity, RF_SW GPIO wrong (GPIO_125).\n");
        }
    }

	strcpy(g_rf_switch_status, "phone_diversity");
}
...
static int wcnss_rf_switch_write_proc(struct file *file, const char __user *buffer, unsigned long count, void *data)
{
    ...
    else if( strncmp(buf, "all_off", 7) == 0 ) {
        wcnss_set_antenna_OFF();
    }
    else if( strncmp(buf, "phone_diversity", 15) == 0 ) {
        wcnss_set_antenna_PHONE_Diversity();
    }
    else {
        printk("[wcnss]: usage, echo {phone/pad_main/pad_wifi} > /proc/rf_switch_status\n");
    }

    return len;
}

static int bt_switch_read_proc(char *page, char **start, off_t off, int count, int *eof, void *data)
{
    int len = 0;
...
static int bt_switch_write_proc(struct file *file, const char __user *buffer, unsigned long count, void *data)
{
    char buf[2];
    unsigned long len = count;
    ...
    if( strncmp(buf, "1", 1) == 0 ) {
        ...
        printk("[wcnss]: usage, echo {1/0} > /proc/bt_status\n");
    }
...
wcnss_wlan_probe(struct platform_device *pdev)
{
    ...
    //ASUS_BSP+++ "for wlan, BT antenna switch with the phone/pad mode"
    /*--------------------------------------------------*/
    struct device *wcnss_wlan_dev = &pdev->dev;
    struct device_node *np = wcnss_wlan_dev->of_node;
    ...
    mutex_init(&g_vreg_lock);

    //ASUS_BSP+++ "for wlan, BT antenna switch with the phone/pad mode"
    /*--------------------------------------------------------------------------------*/
    g_PMIC_GPIO_WL_SW_1 = of_get_named_gpio(np, "qcom,has_wl_sw_1", 0);
    if( !gpio_is_valid(g_PMIC_GPIO_WL_SW_1) ) {
        printk("[wcnss]: gpio_is_valid fail, (g_PMIC_GPIO_WL_SW_1=%d).", g_PMIC_GPIO_WL_SW_1);
    }
    else {
        ret = gpio_request(g_PMIC_GPIO_WL_SW_1, "WL_SW_1");
        if (ret) {
            printk("[wcnss]: gpio_request fail, PMIC_GPIO_8 (g_PMIC_GPIO_WL_SW_1=%d).\n", g_PMIC_GPIO_WL_SW_1);
        }
        else {
            printk("[wcnss]: PMIC_GPIO_8 (g_PMIC_GPIO_WL_SW_1=%d).\n", g_PMIC_GPIO_WL_SW_1);
        }
    }

    g_PMIC_GPIO_WL_SW_2 = of_get_named_gpio(np, "qcom,has_wl_sw_2", 0);
    if( !gpio_is_valid(g_PMIC_GPIO_WL_SW_2) ) {
        printk("[wcnss]: gpio_is_valid fail, (g_PMIC_GPIO_WL_SW_2=%d).", g_PMIC_GPIO_WL_SW_2);
    }
    else {
        ret = gpio_request(g_PMIC_GPIO_WL_SW_2, "WL_SW_2");
        if (ret) {
            printk("[wcnss]: gpio_request fail, PMIC_GPIO_9 (g_PMIC_GPIO_WL_SW_2=%d).\n", g_PMIC_GPIO_WL_SW_2);
        }
        else {
            printk("[wcnss]: PMIC_GPIO_9 (g_PMIC_GPIO_WL_SW_2=%d).\n", g_PMIC_GPIO_WL_SW_2);
        }
    }

    g_ANT_GPIO_114 = of_get_named_gpio(np, "qcom,ant_gpio_114", 0);
    if( !gpio_is_valid(g_ANT_GPIO_114) ) {
        printk("[wcnss]: gpio_is_valid fail, (g_ANT_GPIO_114=%d).", g_ANT_GPIO_114);
    }
    else {
	    ret = gpio_request(g_ANT_GPIO_114, "ANT_GPIO_114");
	    if( ret < 0 ) {
		    printk("[wcnss]: gpio_request fail, (g_ANT_GPIO_114=%d).\n", g_ANT_GPIO_114);
	    }
        else {
            printk("[wcnss]: g_ANT_GPIO_114=%d.\n", g_ANT_GPIO_114);
        }

	    ret = gpio_direction_output(g_ANT_GPIO_114, 1);
	    if( ret < 0 ) {
		    printk("[wcnss]: g_ANT_GPIO_114 output fail.\n");
	    }
        else {
            printk("[wcnss]: g_ANT_GPIO_114 output (1).\n");
        }
    }

    g_ANT_GPIO_115 = of_get_named_gpio(np, "qcom,ant_gpio_115", 0);
    if( !gpio_is_valid(g_ANT_GPIO_115) ) {
        printk("[wcnss]: gpio_is_valid fail, (g_ANT_GPIO_115=%d).", g_ANT_GPIO_115);
    }
    else {
	    ret = gpio_request(g_ANT_GPIO_115, "ANT_GPIO_115");
	    if( ret < 0 ) {
		    printk("[wcnss]: gpio_request fail, (g_ANT_GPIO_115=%d).\n", g_ANT_GPIO_115);
	    }
        else {
            printk("[wcnss]: g_ANT_GPIO_115=%d.\n", g_ANT_GPIO_115);
        }

	    ret = gpio_direction_output(g_ANT_GPIO_115, 0);
	    if( ret < 0 ) {
		    printk("[wcnss]: g_ANT_GPIO_115 output fail.\n");
	    }
        else {
            printk("[wcnss]: g_ANT_GPIO_115 output (0).\n");
        }
    }

    g_ANT_GPIO_124 = of_get_named_gpio(np, "qcom,ant_gpio_124", 0);
    if( !gpio_is_valid(g_ANT_GPIO_124) ) {
        printk("[wcnss]: gpio_is_valid fail, (g_ANT_GPIO_124=%d).", g_ANT_GPIO_124);
    }
    else {
	    ret = gpio_request(g_ANT_GPIO_124, "ANT_GPIO_124");
	    if( ret < 0 ) {
		    printk("[wcnss]: gpio_request fail, (g_ANT_GPIO_124=%d).\n", g_ANT_GPIO_124);
	    }
        else {
            printk("[wcnss]: g_ANT_GPIO_124=%d.\n", g_ANT_GPIO_124);
        }

	    ret = gpio_direction_output(g_ANT_GPIO_124, 0);
	    if( ret < 0 ) {
		    printk("[wcnss]: g_ANT_GPIO_124 output fail.\n");
	    }
        else {
            printk("[wcnss]: g_ANT_GPIO_124 output (0).\n");
        }
    }

    g_ANT_GPIO_127 = of_get_named_gpio(np, "qcom,ant_gpio_127", 0);
    if( !gpio_is_valid(g_ANT_GPIO_127) ) {
        printk("[wcnss]: gpio_is_valid fail, (g_ANT_GPIO_127=%d).", g_ANT_GPIO_127);
    }
    else {
	    ret = gpio_request(g_ANT_GPIO_127, "ANT_GPIO_127");
	    if( ret < 0 ) {
		    printk("[wcnss]: gpio_request fail, (g_ANT_GPIO_127=%d).\n", g_ANT_GPIO_127);
	    }
        else {
            printk("[wcnss]: g_ANT_GPIO_127=%d.\n", g_ANT_GPIO_127);
        }

	    ret = gpio_direction_output(g_ANT_GPIO_127, 1);
	    if( ret < 0 ) {
		    printk("[wcnss]: g_ANT_GPIO_127 output fail.\n");
	    }
        else {
            printk("[wcnss]: g_ANT_GPIO_127 output (1).\n");
        }
    }

    if( g_ASUS_hwID == A86_SR2 ) {
        g_ANT_GPIO_125 = of_get_named_gpio(np, "qcom,ant_gpio_125", 0);
        if( !gpio_is_valid(g_ANT_GPIO_125) ) {
            printk("[wcnss]: gpio_is_valid fail, (g_ANT_GPIO_125=%d).", g_ANT_GPIO_125);
        }
        else {
	        ret = gpio_request(g_ANT_GPIO_125, "ANT_GPIO_125");
	        if( ret < 0 ) {
		        printk("[wcnss]: gpio_request fail, (g_ANT_GPIO_125=%d).\n", g_ANT_GPIO_125);
	        }
            else {
                printk("[wcnss]: g_ANT_GPIO_125=%d.\n", g_ANT_GPIO_125);
            }

	        ret = gpio_direction_output(g_ANT_GPIO_125, 0);
	        if( ret < 0 ) {
		        printk("[wcnss]: g_ANT_GPIO_125 output fail.\n");
	        }
            else {
                printk("[wcnss]: g_ANT_GPIO_125 output (0).\n");
            }
        }
    }
    ...
    if( entry == NULL ) {
        printk("[wcnss]: create_proc_entry fail (rf_switch_status).\r\n");
    }
    else {
        entry->read_proc = wcnss_rf_switch_read_proc;
        entry->write_proc = wcnss_rf_switch_write_proc;
    }

    entry = create_proc_entry("bt_status", 0666, NULL);
    if( entry == NULL ) {
        printk("[wcnss]: create_proc_entry fail (bt_status).\r\n");
    }
    else {
        entry->read_proc = bt_switch_read_proc;
        entry->write_proc = bt_switch_write_proc;
    }
