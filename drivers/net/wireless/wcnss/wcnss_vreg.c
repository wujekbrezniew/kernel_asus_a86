...
/* VDD power of wlan antenna switch */
static struct vregs_info wlan_antenna[] = {
	{"qcom,wlan-ant", VREG_NULL_CONFIG, 2700000, 0, 2700000, 0, NULL},
};

int g_wlan_antenna_vreg = 0;
extern struct mutex g_vreg_lock;
...
void wlan_ant_vregs_on(struct device *dev)
{
    int ret = -1;

    mutex_lock(&g_vreg_lock);

    if( g_wlan_antenna_vreg == 0 ) {
        ret = wcnss_vregs_on(dev, wlan_antenna, ARRAY_SIZE(wlan_antenna));
        printk("[wcnss]: wlan_ant_vregs_on (%d).\n", ret);
    }
    else {
	printk("[wcnss]: wlan_ant_vregs_on.\n");
    }

    g_wlan_antenna_vreg = 1;
    printk("[wcnss]: g_wlan_antenna_vreg=1.\n");

    mutex_unlock(&g_vreg_lock);
}


void wlan_ant_vregs_off(void)
{
    mutex_lock(&g_vreg_lock);

    if( g_wlan_antenna_vreg == 1 ) {
        wcnss_vregs_off(wlan_antenna, ARRAY_SIZE(wlan_antenna));
        printk("[wcnss]: wlan_ant_vregs_off().\n");
    }
    else {
        printk("[wcnss]: wlan_ant_vregs_off.\n");
    }

    g_wlan_antenna_vreg = 0;
    printk("[wcnss]: g_wlan_antenna_vreg=0.\n");
