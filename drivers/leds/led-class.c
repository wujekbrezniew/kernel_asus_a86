		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (strncmp(led_cdev->name, "lcd-backlight", sizeof("lcd-backlight")) == 0){
		...
		if (state >= 0 && state <= 63) {
			ASUSEvtlog("[BKL] level: [0]");
		}
		else if (state >= 64 && state <= 127) {
			ASUSEvtlog("[BKL] level: [1]");
		}
		else if (state >= 128 && state <= 191) {
			ASUSEvtlog("[BKL] level: [2]");
		}
		else if (state >= 192 && state <= 255) {
			ASUSEvtlog("[BKL] level: [3]");
		}
		else {
			ASUSEvtlog("[BKL] level: [not within spec]");
		}
}
