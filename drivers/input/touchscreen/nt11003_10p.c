...
#include <linux/microp_notifier_controller.h>	//ASUS_BSP Freeman+
#include <linux/proc_fs.h>
#include <linux/nt11003.h>
#include <linux/switch.h>
#include "nt11306_firmware.h"

//ASUS_BSP Freeman: add for led icon +++
...
static int rc_cal_fail = 1;		//ASUS_BSP Freeman: some fw update need RC cal default set 1 ( fail ) 
static int firmware_version = 0xFF;
static int tpid = 0xFF;
static char last_info[8];

static int usb_state = 0; //ASUS_BSP Freeman: Get USB state

//ASUS_BSP Freeman : Add report location +++
static unsigned char touch_cunt_old;
static int REPORT_COUNT = 100;
static int finger_count[MAX_FINGER_NUM];
//static int SHOW_ENABLE = 0;
//static int SHOW_DISABLE = 0;
//ASUS_BSP Freeman : Add report location ---
...
//ASUS_BSP Freeman : add for On/Off touch in P05 +++
...
//ASUS_BSP Freeman : add for On/Off touch in P05 ---
static struct wake_lock touch_wake_lock; //ASUS_BSP Freeman: Add for wake_lock when update firmware ++

static int touch_key_back_status = 0,touch_key_home_status = 0,touch_key_menu_status = 0;
static int touch_key_flag = 0;
...
//ASUS_BSP Freeman : Add for HWrst +++
...
//ASUS_BSP Freeman : Add for write RC flag to  asusdata +++
#ifdef ASUS_FACTORY_BUILD
...
//ASUS_BSP Freeman: temporary way to notify usb state +++
...
void led_icon_trigger(struct leddev *leds, int value)
{
	...
	pwm_config_us(leds->pwmb, dutytime, leds->period);
 	pwm_enable(leds->pwmb);
...
static int nt11003_ts_resume(struct i2c_client *client)
{
	unsigned char i2c_swrst_buf[2] = {0x00, 0x5A};	//suspend cmd, (I2C buffer = 255, data = 0)
 	...
 	{
		printk("[Touch_N] Skip %s, ts_inpad : %d,ts_suspend : %d\n", __func__,ts_inpad,ts_suspend);
 		return 0;
 	}
	...
	msleep(150);//ASUS_BSP Freeman: wait soft calibration finish

	//SHOW_ENABLE = 3;
	//SHOW_DISABLE = 3;
	//printk("[Touch_N]  %s SHOW_ENABLE : %d,SHOW_DISABLE : %d\n",__func__,SHOW_ENABLE,SHOW_DISABLE);
	...
	nv_touch_mode(usb_state);// ASUS_BSP Freeman: Add for resume to get USB state
...
//ASUS_BSP Freeman : add for replace early suspend +++
#if defined(CONFIG_FB)
...
//ASUS_BSP Freeman : Add touch deriver attribute +++
...
//ASUS_BSP Freeman: some fw update need RC cal +++
				...
				const char *buf, size_t count)
{
	...
	CTP_I2C_READ(ts->client, 0x01, I2C_Buf, 2);
		...
		msleep(200); 			//ASUS_BSP Freeman : wait soft calibration finish
		nv_touch_mode(usb_state);	//ASUS_BSP Freeman: Add for Get Usb state after rc_calibartion to avoid usb plugin while update FW
	...
	//ASUS_BSP Freeman : do soft_reset either RC K fail
	I2C_Buf[0] = 0x00;
	I2C_Buf[1] = 0x5A;
	CTP_I2C_WRITE(ts->client, 0x7F, I2C_Buf, 2);
...
//ASUS_BSP Freeman : combine attribute +++
...
//ASUS_BSP Freeman : load bin file to updae FW from user-space +++
...
DEVICE_ATTR(rc_calibration, NV_RW_ATTR, rc_calibration_result, rc_calibration); //ASUS_BSP Freeman: fw v23 need RC cal++
//ASUS_BSP Freeman: add for led +++
...
static struct attribute *nt_attrs[] = {
	...
	&dev_attr_rc_calibration.attr, //ASUS_BSP Freeman: fw v23 need RC cal ++
//ASUS_BSP Freeman: add for led +++
...
//ASUS_BSP Freeman: change way to report key ++++
			if (index != 0 && touch_key_flag == 1) {
				if(touch_key_back_status == 2)
				{
					input_report_key(ts->input_dev,KEY_BACK,0);
				}

				if(touch_key_home_status == 2)
				{
					input_report_key(ts->input_dev,KEY_HOME,0);
				}

				if(touch_key_menu_status == 2)
				{
					input_report_key(ts->input_dev,KEY_MENU,0);
				}
				touch_key_back_status = 0;
				touch_key_home_status = 0;
				touch_key_menu_status = 0;
				touch_key_flag = 0;

				//input_mt_sync(ts->input_dev);
			}

			if(touch_key_back_status == 1)
			{
				input_report_key(ts->input_dev,KEY_BACK,0);
				touch_key_back_status = 0;
			}

			if(touch_key_home_status == 1)
			{
				input_report_key(ts->input_dev,KEY_HOME,0);
				touch_key_home_status = 0;
			}

			if(touch_key_menu_status == 1)
			{
				input_report_key(ts->input_dev,KEY_MENU,0);
				touch_key_menu_status = 0;
			}
...
//ASUS_BSP Freeman: add for led icon ---
		...
		else
		{
			...
			if((input_x > 1090 )||(input_y > 1970))	continue;	//ASUS_BSP Freeman : Fix conditition +++

//ASUS_BSP Freeman: change way to report key  ++++
			//if(input_y > 1930 && input_y < 1990){
				//if(input_x > 240 && input_x < 300){
			if(input_y == 1960){
				if(input_x == 270){	
					input_report_key(ts->input_dev,KEY_BACK,1);
					touch_key_back_status = 1;
					if(touch_key_home_status == 1 || touch_key_menu_status == 1)
					{
						touch_key_back_status = 2;
						touch_key_flag = 1;
					}
				}
				//if(input_x > 510 && input_x < 570){
				if(input_x == 540){
					input_report_key(ts->input_dev,KEY_HOME,1);
					touch_key_home_status = 1;
					if(touch_key_back_status == 1 || touch_key_menu_status == 1)
					{
						touch_key_home_status = 2;
						touch_key_flag = 1;
					}
				}
				//if(input_x > 780 && input_x < 840){
				if(input_x == 810){
					input_report_key(ts->input_dev,KEY_MENU,1);
					touch_key_menu_status = 1;
					if(touch_key_home_status == 1 || touch_key_back_status == 1)
					{
						touch_key_menu_status = 2;
						touch_key_flag = 1;
					}
				}
			}
...
END_WORK_FUNC:
	if(!irq_balance){
		...
		{
			SHOW_ENABLE--;
			printk("[Touch_N]  %s SHOW_ENABLE : %d\n",__func__,SHOW_ENABLE);
		}*/
		irq_balance = 1;
	}
}
...
static irqreturn_t nt11003_ts_irq_handler(int irq, void *dev_id)
{
	...
	if(irq_balance){
		...
		{
			SHOW_DISABLE--;
			printk("[Touch_N]  %s SHOW_DISABLE : %d\n",__func__,SHOW_DISABLE);
		}*/
	}
...
//ASUS_BSP Freeman : Parse device tress info +++
...
static int nt11003_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	//ASUS_BSP Freeman : Add FB notify for early suspend +++
	#if defined(CONFIG_FB)
	...
	//ASUS_BSP Freeman: Add attribute +++
	...
	//ASUS_BSP Freeman: add wake lock
	wake_lock_init(&touch_wake_lock, WAKE_LOCK_SUSPEND, "touch_wake_lock");

	register_microp_notifier(&touch_mp_notifier_nv);
	notify_register_microp_notifier(&touch_mp_notifier_nv, "nt11003_10p"); //ASUS_BSP Freeman+++
	//ASUS_BSP Freeman : add for On/Off touch in P05 ---
...
static void __exit nt11003_ts_exit(void)
{
	...
	notify_unregister_microp_notifier(&touch_mp_notifier_nv, "nt11003_10p"); //ASUS_BSP Freeman+++
