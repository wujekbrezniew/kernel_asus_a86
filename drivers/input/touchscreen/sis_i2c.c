...
//#define CAL_POINT_PROCESS_TIME
#define TOTAL_WRITE_COUNT 1463
...
//ASUS_BSP Freeman : force release fingers +++
...
//ASUS_BSP Freeman : get boot flag +++
...
//ASUS_BSP Freeman : add for I2C_STRESS_TEST +++
...
*/
	...
	if(ret < 0 )
	{
		//ASUS_BSP Freeman  +++
		if(ret == -ENOTCONN){
			sis_debug(NO_DEBUG, "[Touch][sis9257] sis_ReadPacket: i2c slave is not connect\n");
			return ret;
		}
		//ASUS_BSP Freeman  ---
		sis_debug(NO_DEBUG, "[Touch][sis9257] sis_ReadPacket: first i2c transfer error, ret = %d\n", ret);
		return ret;
	}
	...
	else 								//TOUCH_SERIAL
	{
		...
		if(touchnum > 5)
		{
			...
			if(ret < 0)
			{
				sis_debug(NO_DEBUG, "[Touch][sis9257] sis_ReadPacket: second i2c transfer error, ret = %d\n", ret);
				return ret;
			}
...
*/
	//g_bIsRealTouchUp = true; //ASUS_BSP Freeman +++
...
static void sis_ts_work_func(struct work_struct *work)
{
	...
	if (ret < 0) //Error fingers' number or Unknow bytecount
	{
		sis_debug(DEBUG_POWER, "[Touch][sis9257] work_func: Error fingers' number or Unknow bytecount\n");
		goto err_free_allocate;
	}
	else if (ret == 0)
	{
...
//ASUS_BSP Freeman : add first finger down log +++
	if(fingers != 0) {
		g_work_count++;
		if(g_work_count == 1)
...
//label_send_report:
    /* Report co-ordinates to the multi-touch stack */
		...
		for(i = 0; ((i < TPInfo->fingers) && (i < MAX_FINGERS)); i++)
		{
			...
			if (i == (TPInfo->fingers -1) && all_touch_up == true)
			{
				...
				g_work_count = 0; //ASUS_BSP Freeman : reset work_count +++

				input_mt_sync(ts->input_dev);
			}
		}

		//ASUS_BSP Freeman : This is error-tolerance code. driver won't run here in normal case. +++
		if(TPInfo->fingers == 0)
		{
			input_mt_sync(ts->input_dev);
		}
	...
	//ASUS_BSP Freeman  measure the time spended in driver +++
#ifdef CAL_POINT_PROCESS_TIME
	if (irq_count%100 == 0){
		do_gettimeofday(&tv);
...
static irqreturn_t sis_ts_irq_handler(int irq, void *dev_id)
{
	...
	if(g_bIsSleep || !g_bIsPadAttach){
		sis_debug(DEBUG_POWER, "isSleep:%d, isPadAttach:%d\n", g_bIsSleep, g_bIsPadAttach);
		return IRQ_NONE;
	}
	//ASUS_BSP Freeman ---
	...
	//ASUS_BSP Freeman : measure time spended in driver +++
#ifdef CAL_POINT_PROCESS_TIME
	if (ret){
		irq_count++;
		if (irq_count%100 == 0){
...
//ASUS_BSP Freeman : For share ISR, we cannot request GPIO +++
#if 0
static int initial_irq(void)
{
	int ret = 0;
	static bool bIsGpioReady = false; //ASUS_BSP Freeman +++
...
//ASUS_BSP Freeman : Add for reset FW, show FW ID, sysfs +++
static ssize_t sis_show_fw_id(struct device *dev, struct device_attribute *devattr,char *buf)
{
	...
	if(g_fw_id[0]==0x97&&g_fw_id[1]==0x31&&g_fw_id[2]==0x30&&g_fw_id[3]==0x31)
	...
	if(ts_bak){
		while (retry != 0){
			if(g_bIsPadAttach){
				...
				if(ret < 0){
					sis_debug(NO_DEBUG, "[Touch][sis9257] sis_show_fw_id:#%d Failed to read FW ID\n", (6-retry));
				}else {
					sis_debug(DEBUG_POWER, "[Touch][sis9257] sis_show_fw_id:#%d %c%02x%02x%02x%02x\n", (6-retry), fw_id[4], fw_id[8], fw_id[9], fw_id[10], fw_id[11]);

					if(fw_id[0]==0x97&&fw_id[1]==0x31&&fw_id[2]==0x30&&fw_id[3]==0x31)
						break;
				}
			}else{
	...
	if(ts_bak){
		while (retry != 0){
			if(g_bIsPadAttach){
				...
				if(ret < 0){
					sis_debug(NO_DEBUG, "[Touch][sis9257] sis_show_full_fw_id: Failed to read FW ID, retry = %d\n", (6-retry));
				}else {
	...
	if(ts_bak){
		while (retry != 0){
			if(g_bIsPadAttach){
				...
				if(ret < 0){
					sis_debug(NO_DEBUG, "[Touch][sis9257] sis_show_boot_flag:#%d Failed to read Boot Flag\n", (6-retry));
				}else {
...
int sis_cdev_release(struct inode *inode, struct file *filp)
{
	...
	g_updateProgress = TOTAL_WRITE_COUNT; //ASUS_BSP Freeman +++
...
//ASUS_BSP Freeman : Remove original suspend/resume functions because FW doesn't support. +++
...
{
	if(g_bIsPadAttach)
		return sprintf(buf, "%c%02x%02x", g_fw_id[4], g_fw_id[9], g_fw_id[11]);
...
//ASUS_BSP Freeman : add for I2C_STRESS_TEST ---
	...
	mutex_init(&ts->mutex); //ASUS_BSP Freeman +++

//ASUS_BSP Freeman : add switch_dev for AMAX update interface +++
...
static struct i2c_driver sis_ts_driver = {
	...
	.driver = {
		.name	= SIS_I2C_NAME,
		.of_match_table = sis_i2c_table, //ASUS_BSP Freeman +++
	},
};
...
#ifdef CONFIG_EEPROM_NUVOTON
//ASUS_BSP Freeman : workaround for Pad touch may hold intr gpio low when init+++
...
#ifdef ASUS_DEBUG_LOG
	ASUSEvtlog("[Touch][sis9257] Touch initial successfully after inserting in pad, FWID: %c%x%x%x%x, BOOT_FLAG: %x%x%x%x, TPID: %d\n", g_fw_id[4], g_fw_id[8], g_fw_id[9], g_fw_id[10], g_fw_id[11], g_boot_flag[0], g_boot_flag[1], g_boot_flag[2], g_boot_flag[3], g_tp_id);
#endif

//ASUS_BSP Freeman : workaround for Pad touch may hold intr gpio low when init+++
	forceDoMicropIrqHandler();
//ASUS_BSP Freeman : workaround for Pad touch may hold intr gpio low when init---
	sis_debug(DEBUG_POWER, "[Touch][sis9257] attach_padstation_work()--%d\n", gpio_get_value(9));
	return;
...
err_virtual_remove:
	//TODO virtual remove
	sis_debug(NO_DEBUG, "[Touch][sis9257] %s: get firmware information occurs soem error\n", __func__);
*/
}
...
{
	sis_debug(DEBUG_INFO, "[Touch][sis9257] touch_attach_padstation_pad()++\n");
...
{
	sis_debug(DEBUG_INFO, "[Touch][sis9257] touch_detach_padstation_pad()++\n");
...
static int touch_mp_event(struct notifier_block *this, unsigned long event, void *ptr)
{
        ...
        case P01_ADD:
                sis_debug(DEBUG_INFO, "[Touch][sis9257] P03_ADD++\r\n");
        ...
        case P01_REMOVE:
                sis_debug(DEBUG_INFO, "[Touch][sis9257] P03_REMOVE++\r\n");
