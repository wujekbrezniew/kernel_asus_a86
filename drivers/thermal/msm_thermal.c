...
/* +++ ASUS_BSP gauss_li For Thermal change profile will stuck by limited_max_freq */
#include <asm/uaccess.h>
#include <linux/proc_fs.h>
#define ASUS_SA_FILE "ASUS_SA"
/* --- ASUS_BSP gauss_li For Thermal change profile will stuck by limited_max_freq */
...
#define CPU_MAX_FREQ 2265600	/* --- ASUS_BSP gauss_li For Thermal change profile will stuck by limited_max_freq */
		...
		unsigned long event, void *data)
{
	...
	uint32_t max_freq_req;
	uint32_t min_freq_req;
/* +++ ASUS_BSP gauss_li For Thermal change profile will stuck by limited_max_freq */
	if(asus_sa_enable == 1) {
		cpus[policy->cpu].limited_max_freq = CPU_MAX_FREQ;
		max_freq_req = CPU_MAX_FREQ;
	} else {
...
int __init msm_thermal_late_init(void)
{
	...
	interrupt_mode_init();
	proc_create(ASUS_SA_FILE, S_IRWXUGO, NULL, &create_asus_proc_file);/* --- ASUS_BSP gauss_li For Thermal change profile will stuck by limited_max_freq */
	return 0;
}
