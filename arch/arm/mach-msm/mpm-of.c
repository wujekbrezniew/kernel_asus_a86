...
static int msm_mpm_debug_mask __initdata = 1;
module_param_named(
...
static inline int __refdata mpm_irq_domain_linear_size(struct irq_domain *d)
{
	return d->revmap_data.linear.size;
}

static inline int __refdata mpm_irq_domain_legacy_size(struct irq_domain *d)
{
